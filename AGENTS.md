...
Internal Dashboard implementation is complete and integrated into `main` by
fast-forward with no merge commit. The retained
`feature/m3.9-internal-dashboard` branch is the completed implementation
branch; WU1–WU4 are complete and the Admin Shell design-adjustment checkpoint
is `NO CHANGE REQUIRED` for the current baseline. M3.10 preparation is next
and not started.
