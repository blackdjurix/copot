...
MR.1 Installation Refinement: WU1–WU4 are COMPLETE AND CLOSED at `docs/36_mr_1_installation_refinement_contract.md`; WU4 implementation, technical validation, functional installation validation, and human UI acceptance are PASS. The authoritative five-WU installer topology remains Requirements → Database → Administrator & Site → Review & Install → Installation Result; the former WU4 Module-selection feature is superseded and removed from MR.1 installer UX. The shared installer UI foundation remains authoritative, with generic presentation consumed from shared primitives. Review & Install remains the first COPOT schema/Admin/Site/baseline-Module mutation boundary; database-container creation at Database is the unchanged explicit capable-environment exception. WU5 Cross-Step Responsive, Accessibility & Human Acceptance is NOT STARTED / NEXT ACTIVE IMPLEMENTATION TARGET. MR.1 branch lifecycle is main-only / no-op. The Pre-MR.1 Correctness Gate is SATISFIED / VALIDATED: Blocker A `DB_NAMESPACE` persistence is RESOLVED / VALIDATED and Blocker B Change Database / Finalize progression state is CLASSIFIED / NOT A DEFECT. Cross-Fileset Upgrade Ownership Proof Gap remains a separate ARCHITECTURE / PRODUCT INVESTIGATION REQUIRED item and is non-blocking by default.
