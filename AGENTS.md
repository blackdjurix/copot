...
M3.6 Navigation Manager preparation is `NRP CONFIRMED`. Work Unit 1 — Contract, Provisioning, and Compatibility Foundation implementation and primary validation are complete; focused compatibility passed 23 assertions and WU1 is `NRP CANDIDATE`. WU2 is not started and is the next separately authorized gate. Full M3.6 is `NRP NOT REACHED`. The preparation contract is `docs/22_m3_6_navigation_manager_contract.md`. Navigation owns its domain, Themes declare `navigation_locations` and own presentation, Content remains the baseline optional target provider, production Taxonomy targets remain deferred, and `AdminNavigation` remains separate.
