...
`14e7ed09681b9f60d57853826a12359d8e2efd8a`. WU3 — Core Primary Navigation
Admin Baseline is implementation-complete, technically validated, AI-accepted,
and human/product-accepted on `feature/webcore-navigation-wu3-baseline` at
...
MR.1 Installation Refinement: WU1–WU5 and full MR.1 are COMPLETE AND CLOSED at `docs/36_mr_1_installation_refinement_contract.md`; WU5 implementation, technical/objective validation, and human acceptance are PASS. The authoritative five-phase installer flow remains Requirements → Database → Administrator & Site → Review & Install → Installation Result; the former WU4 Module-selection feature is superseded and removed from MR.1 installer UX. The shared installer UI foundation remains authoritative, with generic presentation consumed from shared primitives; accepted mobile behavior is a full-screen shell without card treatment, with only phase content scrolling when necessary. Shared field errors/help are programmatically associated with their controls and invalid controls expose invalid state. A successful commit creates installation identity before Installation Result; Continue to Admin does not create it. Review & Install remains the first COPOT schema/Admin/Site/Webcore-baseline mutation boundary; optional Module operations remain separate lifecycle actions; database-container creation at Database is the unchanged explicit capable-environment exception. MR.1 branch lifecycle is main-only / no-op. The Pre-MR.1 Correctness Gate is SATISFIED / VALIDATED: Blocker A `DB_NAMESPACE` persistence is RESOLVED / VALIDATED and Blocker B Change Database / Finalize progression state is CLASSIFIED / NOT A DEFECT. Cross-Fileset Upgrade Ownership Proof Gap remains a separate ARCHITECTURE / PRODUCT INVESTIGATION REQUIRED item and is non-blocking by default.
