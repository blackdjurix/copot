...
MR.1 Installation Refinement: PROMOTED / IMPLEMENTATION IN PROGRESS at `docs/36_mr_1_installation_refinement_contract.md`; authoritative six-WU staged-installation topology: WU1 Installer Shell, Requirements & Navigation Framework is IMPLEMENTATION IN PROGRESS; WU2 Staged Installation Plan & Database Decision, WU3 Administrator & Site Staging, WU4 Optional Core Module Selection, WU5 Final Review, Installation Commit & Result, and WU6 Cross-Step Responsive, Accessibility & Human Acceptance are NOT STARTED. Review & Install in WU5 is the first COPOT schema/Admin/Site/optional-Module mutation boundary; database-container creation at Database is the explicit capable-environment exception. The Pre-MR.1 Correctness Gate is SATISFIED / VALIDATED: Blocker A `DB_NAMESPACE` persistence is RESOLVED / VALIDATED and Blocker B Change Database / Finalize progression state is CLASSIFIED / NOT A DEFECT. Cross-Fileset Upgrade Ownership Proof Gap remains a separate ARCHITECTURE / PRODUCT INVESTIGATION REQUIRED item and is non-blocking by default.
