...
$scenario = $makeScenario('copot-iu2-wu5-after-lifecycle');
try { $result = (new LegacyReconciliationFinalizer($scenario[6], new TargetPackageIntegrityVerifier(), new \Copot\Core\DatabaseHealthVerifier(), new CoreMigrationHealthVerifier(), new \Copot\Core\RuntimeHealthVerifier()))->finalize($scenario[3], $scenario[4], $scenario[5], new WebcoreApplyResult(WebcoreApplyResult::COMPLETED), $scenario[10], $scenario[7], $scenario[9], new \Copot\Core\LiveTreePathGuard($scenario[1]), $scenario[8], new CoreMigrationRegistry('empty', []), ['runtime' => static fn (): bool => true], null, static function (): void { throw new RuntimeException('forced final verification interruption'); }); $assert($result->status() === LegacyReconciliationFinalizationResult::FAILED, 'Post-lifecycle verification interruption was reported as success.'); $assert($scenario[6]->read($scenario[4]->recoveryIdentity())->state() === RecoveryLifecycleState::RESTORE_REQUIRED, 'Post-lifecycle verification interruption did not preserve recovery-required state.'); $assert($scenario[7]->readMarker()['version'] === '0.13.0' && $scenario[9]->read() instanceof \Copot\Core\CommittedLifecycleState, 'Post-lifecycle interruption did not preserve the committed writes for restore.'); } finally { $remove($scenario[0]); }
