...
$assert(str_contains($bootstrap, 'SystemHealthAggregator') && str_contains($bootstrap, 'new SiteSettingsModuleHealthProducer($app)') && str_contains($bootstrap, "can('modules.manage')") && !str_contains($bootstrap, 'findings() === []') && str_contains($bootstrap, '?SystemHealthReport'), 'Module Health production composition does not gate visibility or preserve operational/empty evidence safely.');
...
$assert(str_contains($detailView, 'admin-button admin-button--secondary') && str_contains($detailView, 'href="<?= $escape($inventoryPath) ?>"') && str_contains($detailView, 'Module Overview') && str_contains($detailView, "What's New") && str_contains($detailView, 'Issues &amp; Guidance') && str_contains($detailView, 'Technical evidence'), 'Site Settings Module Detail hierarchy does not preserve canonical Back navigation or the accepted operator-facing sections.');
$assert(str_contains($authority, 'ReleaseMetadataValidator::validate') && str_contains($authority, "'modules/' . \$name . '/release.json'") && !str_contains($authority, 'CHANGELOG'), 'Module Detail What\'s New does not use the existing validated Module release metadata authority.');
$assert(str_contains($css, '.admin-module-detail-header') && str_contains($css, '.admin-module-detail-header {') && preg_match('/\\.admin-module-detail-whats-new\\s*\\{\\s*margin-bottom:\\s*var\\(--admin-space-6\\);/', $css) === 1 && str_contains($css, '.admin-module-detail-disclosure > summary') && str_contains($css, 'flex-direction: column;'), 'Responsive Module Detail identity header, What\'s New separation, or progressive disclosure layout is missing.');
...
$assert($emptyHealth?->status() === \Copot\Core\SystemHealthOverallStatus::OPERATIONAL && $emptyHealth?->findings() === [], 'An authorized zero-finding Module-only producer did not produce an Operational report.');
...
$assert(str_contains($settingsView, '$canUpdateSettings') && str_contains($settingsView, '$canManageModules') && str_contains($settingsView, 'moduleItems') && !str_contains($settingsView, "'security' => 'Security'") && !str_contains($settingsView, "'email' => 'Email'"), 'Site Settings capability-backed area composition is missing.');
...
$assert($detailHeaderEnd !== false && str_contains($detailHeader, 'Form Manager') && !str_contains($detailHeader, 'form-manager') && strpos($detailHtml, 'Module Overview') < strpos($detailHtml, "What's New") && strpos($detailHtml, "What's New") < strpos($detailHtml, 'Issues &amp; Guidance'), 'Site Settings Module Detail does not show the human name first with What\'s New in the required hierarchy.');
$assert(str_contains($detailHtml, 'module-whats-new-title') && str_contains($detailHtml, '&amp;lt;not markup&amp;gt;'), 'Validated Module What\'s New entries are not safely rendered.');
...
$assert(str_contains($normalDetailHtml, "What's New information is unavailable.") && str_contains($normalDetailHtml, 'No issues detected.') && str_contains($normalDetailHtml, '>Disable</button>') && !str_contains($normalDetailHtml, 'already_installed') && !str_contains($normalDetailHtml, 'already_enabled') && !str_contains($normalDetailHtml, 'enabled_module') && !str_contains($normalDetailHtml, 'Stored path available</dt><dd>Yes'), 'Unavailable What\'s New or normal lifecycle inapplicability was presented as an issue or changed action visibility.');
