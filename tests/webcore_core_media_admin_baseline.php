...
try {
    ...
    $assert(str_contains($body($app->run(new Request('GET', $mediaPath))), 'Core Media inventory'), 'Core inventory view was not rendered.');
    ...
    $assert($status($app->run(new Request('POST', $app->adminUrl()->childUrl("media/{$id}/title"), [], ['title' => 'missing csrf']))) === 419, 'Title update accepted missing CSRF.');
    $titleResponse = $app->run(new Request('POST', $app->adminUrl()->childUrl("media/{$id}/title"), [], ['_token' => $app->csrf()->token(), 'title' => 'Renamed'])); $assert($status($titleResponse) === 302, 'Bounded title update failed.');
