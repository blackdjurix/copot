...
try {
    ...
    $assert(str_contains($postUploadHtml, 'data-media-preview-open') && str_contains($postUploadHtml, 'role="button"') && str_contains($postUploadHtml, 'admin-core-media.js') && str_contains($postUploadHtml, '>Upload Media</a>'), 'Core Media row activation, quick-preview surface, or accepted upload label is missing.');
    ...
    $assert(str_contains($coreCss, 'tr[data-media-preview-open]:hover > td') && str_contains($coreCss, 'tr[data-media-preview-open]:focus-visible > td') && str_contains($coreCss, 'color-mix(in srgb, var(--admin-color-primary) 7%, var(--admin-color-surface))') && str_contains($coreCss, 'position: absolute') && str_contains($coreCss, 'border: 0') && str_contains($coreCss, 'background: transparent') && str_contains($coreCss, 'grid-template-columns: 1fr'), 'Core Media acceptance affordances do not provide whole-row hover/focus feedback, a transparent dialog close control, or stacked details.');
