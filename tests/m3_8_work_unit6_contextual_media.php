...
try {
    ...
    $profile = (string) file_get_contents($base . '/modules/media/Services/MediaFeaturedProfile.php'); $picker = (string) file_get_contents($base . '/public/admin-assets/js/content-media-picker.js'); $show = (string) file_get_contents($base . '/modules/content/views/show.php'); $form = (string) file_get_contents($base . '/modules/content/views/admin/form.php'); $assert(str_contains($profile, "'profile' => 'content.featured'") && str_contains($profile, "'responsive_widths' => [640, 960, 1280]"), 'Featured profile bounds are not locked.'); $assert(str_contains($picker, 'processCrop') && str_contains($picker, 'Upload'), 'Picker prepare/upload flow is incomplete.'); $assert(str_contains($picker, 'currentDescriptor && currentDescriptor.url') && str_contains($picker, 'cropImage.onload =') && str_contains($picker, 'announce(message)'), 'Re-crop does not restore the current preview or announce processing failures.'); $assert(str_contains($show, 'featuredMedia') && str_contains($show, 'srcset') && str_contains($form, 'aria-live'), 'Public featured rendering or accessible feedback contract is missing.');
    ...
    $assert(str_contains($picker, 'data-media-picker-crop-y') && str_contains($picker, 'pointerdown') && str_contains($picker, 'cropY.value') && str_contains($picker, 'cropZoom') && str_contains($picker, '1280'), 'Two-dimensional bounded crop positioning is missing.');
