...
$assert(str_contains($layout, "pageTypeValue") && str_contains($layout, "['homepage', 'general', 'system']") && str_contains($layout, "\$pageType === 'homepage'") && str_contains($layout, 'aspect-ratio: 3 / 4') && str_contains($layout, 'border-radius: 0;'), 'Built-in Public View page taxonomy or responsive Homepage Hero presentation is missing.');
$assert(str_contains($layout, 'width: min(900px, 100%)') && str_contains($layout, 'border-radius: 0 0 12px 12px') && str_contains($layout, '.builtin-site-hero { width: 100%; max-width: 900px;'), 'Homepage public frame width or desktop Hero corner treatment is incorrect.');
$assert(str_contains($layout, 'builtin-site-header__bar') && str_contains($layout, 'width: 100%; height: 4px') && str_contains($layout, 'background: var(--builtin-main)') && !str_contains($layout, 'builtin-site-page-strip'), 'Built-in Public View shared header color bar boundary is incorrect.');
$assert(str_contains($layout, '.builtin-site-main--general .builtin-site-content { margin-top: 0; border-radius: 0 0 12px 12px; }'), 'General Page content is not attached to the shared header bar.');
$assert(str_contains($layout, 'var(--builtin-main-strong)') && !str_contains($layout, 'var(--builtin-accent-strong)'), 'Built-in Public View still consumes stale accent color tokens.');
$assert(str_contains($contentView, 'builtin-site-breadcrumb') && str_contains($contentView, 'builtin-site-featured-image') && str_contains($contentView, 'builtin-site-page__intro'), 'General Page editorial presentation is missing.');
$assert(str_contains($contentView, "=== 'article'") && !str_contains($contentView, 'if (($entry[\'published_at\'] ?? null) !== null):'), 'General Page publication metadata is not content-type aware.');
$assert(str_contains($layout, 'font-size: clamp(1.75rem, 4vw, 2rem)') && !str_contains($layout, 'font-size: clamp(1.75rem, 4vw, 2.5rem)') && !str_contains($layout, 'font-size: 2rem;'), 'Built-in Public View does not use one unified bounded responsive title scale.');
$assert(str_contains($layout, '.builtin-site-content h1 { margin: 0 0 0px; color: #17202a; font-size: clamp(1.75rem, 4vw, 2rem);'), 'Built-in Public View H1 bottom margin is not zero.');
$assert(!str_contains($layout, 'border-bottom: 1px solid #d9e0e7') && !str_contains($contentView, 'builtin-site-page__title--article'), 'Article-specific title styling or the removed header seam remains.');
$assert(str_contains($layout, 'currentPath') && str_contains($layout, 'aria-current="page"') && str_contains($layout, 'builtin-site-nav__link.is-active::after') && str_contains($layout, 'var(--builtin-main)'), 'Primary Navigation active-state presentation is missing or not Main Color driven.');
$assert(strpos($contentView, 'builtin-site-breadcrumb') < strpos($contentView, '<h1') && strpos($contentView, '<h1') < strpos($contentView, 'builtin-site-page__meta') && strpos($contentView, 'builtin-site-page__meta') < strpos($contentView, 'builtin-site-page__intro') && strpos($contentView, 'builtin-site-page__intro') < strpos($contentView, 'builtin-site-featured-image') && strpos($contentView, 'builtin-site-featured-image') < strpos($contentView, '<div>'), 'General Page editorial blocks are not ordered Breadcrumb, H1, article metadata, intro, featured image, body.');
$assert(str_contains($layout, '.builtin-site-page__meta { margin: 0 0 28px; color: #596674 !important; font-size: .9rem !important; }') && str_contains($layout, '.builtin-site-page__intro { font-size: 1.2rem !important; }') && !str_contains($layout, '.builtin-site-page__intro { margin:'), 'Public metadata or intro spacing does not match the accepted rules.');
...
$assert(str_contains($css, '.site-settings-identity__hero .site-settings-hero-actions .admin-button { width: 7rem; }') && str_contains($css, '.site-settings-hero-actions .admin-button[hidden] { display: none; }') && str_contains($view, 'data-site-settings-hero-open<?= !empty($values[\'hero_media\']) ? \' hidden\' : \'\' ?>') && str_contains($view, 'data-site-settings-hero-clear<?= empty($values[\'hero_media\']) ? \' hidden\' : \'\' ?>'), 'Hero action sizing or state visibility is not stable across viewport/state changes.');
