...
$navigationCss = $read('public/assets/navigation.css');
$navigationJs = $read('public/assets/navigation.js');
...
$assert(str_contains($registry, "'homepage_content'") && str_contains($siteSettings, "homepage_content_type") && str_contains($view, 'Article Collection') && str_contains($view, 'homepage_content_page'), 'Homepage Content assignment projection is missing.');
$assert(str_contains($siteSettings, "'homepage_content', \$homepageContent, 'json'") && str_contains($web, "'homepage_content'") && str_contains($web, 'publishedArticles()'), 'Homepage Content persistence or render-only source resolution is missing.');
$assert(str_contains($view, 'None</option>') && str_contains($view, 'Page</option>') && str_contains($view, 'Article Collection</option>') && str_contains($view, 'homepage_content_type'), 'Homepage Content does not expose the exact bounded choices.');
$assert(str_contains($web, "'homepageContent' => \$homepageContent") && str_contains($homeView, 'homepageContent') && !str_contains($homeView, 'builtin-site-home'), 'Homepage is not a render-only assignment consumer without a Site Name/Tagline body fallback.');
$assert(str_contains($layout, "if (\$pageType !== 'homepage' || trim((string) (\$content ?? '')) !== '')") && str_contains($layout, 'builtin-site-content--<?= htmlspecialchars($pageType'), 'Homepage None state still renders a decorative content container.');
...
$assert(str_contains($layout, '.builtin-site-content h1 { margin: 0 0 18px; color: #17202a; font-size: clamp(1.75rem, 4vw, 2rem);') && str_contains($layout, '.builtin-site-content h1.builtin-site-content__title--detail { margin-bottom: 0; }'), 'Built-in Public View generic/detail H1 spacing is incorrect.');
...
$articleIntro = strpos($contentView, 'builtin-site-page__intro builtin-site-page__intro--detail');
$assert(strpos($contentView, 'builtin-site-breadcrumb') < strpos($contentView, '<h1') && strpos($contentView, '<h1') < strpos($contentView, 'builtin-site-page__meta') && strpos($contentView, 'builtin-site-page__meta') < strpos($contentView, 'builtin-site-featured-image') && strpos($contentView, 'builtin-site-featured-image') < $articleIntro && $articleIntro < strpos($contentView, '<div>'), 'General Page editorial blocks are not ordered Breadcrumb, H1, article metadata, featured image, intro, body.');
...
$assert(str_contains($web, "'breadcrumbs' => []") && str_contains($web, "'currentPath' => '/articles'") && str_contains($articles, 'builtin-site-article-item') && str_contains($articles, 'featured_media_id'), 'Homepage Page breadcrumb suppression or shared Article Collection presentation is missing.');
$assert(str_contains($contentView, '$hideTitle') && str_contains($contentView, '!$hideTitle'), 'Homepage Page projection does not suppress only its projected title.');
$assert(str_contains($css, 'site-settings-page [data-homepage-content-page-field][hidden]') && str_contains($layout, 'builtin-site-article-item__body h2 { margin: 0; }'), 'Scoped Homepage visibility or Article Collection heading treatment is missing.');
...
$assert(str_contains($contentView, 'builtin-site-page__intro--detail'), 'Page and Article detail excerpts do not share the detail presentation.');
$assert(str_contains($layout, '.builtin-site-content h1.builtin-site-content__title--detail + .builtin-site-featured-image { margin-top: 16px; }') && str_contains($layout, '.builtin-site-page__intro--detail { font-style: italic; }'), 'Detail title/image spacing or shared excerpt styling is missing.');
$pageIntro = strpos($contentView, 'builtin-site-page__intro builtin-site-page__intro--detail');
...
$assert(str_contains($navigationCss, '.builtin-site-nav:not(.is-open) { display: none; }') && str_contains($navigationCss, '.builtin-site-nav.is-open {') && str_contains($navigationCss, 'display: block;'), 'Mobile navigation open/closed presentation is missing.');
...
$assert(str_contains($layout, '.builtin-site-page__meta + .builtin-site-featured-image { margin-top: 16px; }'), 'Single Article metadata-to-featured-image spacing is missing.');
