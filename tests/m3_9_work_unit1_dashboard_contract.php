...
namespace {
    ...
    $assert((bool) preg_match('/M3\.9 WU1:\s+COMPLETE/', $contract) && (bool) preg_match('/M3\.9 WU2:\s+COMPLETE/', $contract) && (bool) preg_match('/M3\.9 WU3–WU4:\s+NOT STARTED/', $contract), 'M3.9 lifecycle status is not recorded correctly.');
    $assert(str_contains($contract, 'feature/m3.9-internal-dashboard') && str_contains($contract, 'WU3–WU4 have not started'), 'M3.9 branch status is not recorded correctly.');
