...
namespace {
    ...
    $assert((bool) preg_match('/M3\.9 WU1:\s+COMPLETE/', $contract) && (bool) preg_match('/M3\.9 WU2:\s+COMPLETE/', $contract) && (bool) preg_match('/M3\.9 WU3:\s+COMPLETE/', $contract) && (bool) preg_match('/M3\.9 WU4:\s+COMPLETE/', $contract) && (bool) preg_match('/M3\.9 implementation:\s+COMPLETE/', $contract), 'M3.9 lifecycle status is not recorded correctly.');
    $assert(str_contains($contract, 'feature/m3.9-internal-dashboard') && str_contains($contract, 'feature-branch lifecycle is CLOSED') && str_contains($contract, 'local and remote implementation branches are deleted') && str_contains($contract, 'integrated into `main` by fast-forward'), 'M3.9 branch status is not recorded correctly.');
