...
try {
    ...
    $workspaceBody = $body($empty); $assert(str_contains($workspaceBody, 'data-navigation-workspace') && str_contains($workspaceBody, 'Navigation item') && !str_contains($workspaceBody, 'name="label"') && !str_contains($workspaceBody, 'name="target_choice"') && !str_contains($workspaceBody, 'Target kind') && !str_contains($workspaceBody, 'Target reference') && str_contains($workspaceBody, 'disabled'), 'Master-detail empty state is incorrect.'); $createView = $app->run(new Request('GET', $navigation, ['create' => '1'])); $assert($status($createView) === 200 && str_contains($body($createView), 'Add navigation item') && str_contains($body($createView), 'name="target_choice"'), 'Create mode did not remain in the workspace.');
