...
try {
    ...
    $workspaceBody = $body($empty); $assert(str_contains($workspaceBody, 'data-navigation-workspace') && str_contains($workspaceBody, 'name="target_choice"') && !str_contains($workspaceBody, 'Target kind') && !str_contains($workspaceBody, 'Target reference') && str_contains($workspaceBody, 'disabled'), 'Master-detail target surface or empty state is incorrect.'); $createView = $app->run(new Request('GET', $navigation, ['create' => '1'])); $assert($status($createView) === 200 && str_contains($body($createView), 'Add navigation item'), 'Create mode did not remain in the workspace.');
    $token=$app->session()->csrfToken(); $create=$app->run(new Request('POST',$app->adminUrl()->childUrl('navigation/items'),[],['_token'=>$token,'label'=>'Home','target_choice'=>'custom','custom_url'=>'/','is_visible'=>'1'])); $assert($status($create)===302 && str_contains($headers($create)['Location'] ?? '', '?item='), 'Custom URL item creation failed.'); $selectedView = $app->run(new Request('GET', $navigation, ['item' => '1'])); $assert($status($selectedView) === 200 && str_contains($body($selectedView), 'value="Home"'), 'Selected item did not populate the detail card.');
