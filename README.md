...
M3.4 Content Manager is complete on `main`; Batch 6 and full M3.4 are `NRP CONFIRMED`. M3.R1 preparation is active on `feature/m3.r1-admin-shell-retouch-1`; its implementation has not started. M3.5 has not started and remains blocked until M3.R1 closes. Release, tag, and publication have not started and remain separately authorized.
