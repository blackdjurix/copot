...
Current WU6 state: the approved Content featured-Media picker, consumer-scoped
...
`feature/m3.8-media-library`. Content is the first active Media consumer;
...
started; full M3.8 remains `NRP NOT REACHED`.
...
functional/browser acceptance are complete while full M3.8 remains `NRP NOT REACHED`. Older M3.7 preparation checkpoint statements
...
`NRP CONFIRMED`. Manual upload accepts one file per submission; drag-and-drop
...
`NRP NOT REACHED`. The contract is
...
and full M3.6 is `NRP CONFIRMED`. M3.7 WU1–WU6 are complete, objectively accepted, and integrated into `main` at `667ae1f0dbb8079ea0420a107bc1795c43cc5bea`; feature containment and branch cleanup passed. Full M3.7 is `NRP CONFIRMED`. The real-settings Theme Settings/color-control spot-check is deferred and non-blocking because the bundled Default Theme declares no settings. M3.8 WU5 presentation refinement is implemented and focused-validated; AI acceptance and human visual/browser acceptance are `PASS` for bounded cards, grid presentation, card-to-preview interaction, preview overlay usability, and the Admin/public action boundary. WU5 is finally closed and `NRP CONFIRMED`. Manual upload accepts one file per submission; drag-and-drop and multiple-file/batch upload remain unsupported future Media enhancements and are not WU5 blockers; video remains outside the current M3.8 contract. WU6 implementation, focused validation, and human functional/browser acceptance are complete on this branch. WU7 is not started and unauthorized. Full M3.8 is `NRP NOT REACHED`.
