...
`NRP CONFIRMED`; WU6 preparation/audit is the next documentation-only target,
while full M3.8 remains `NRP NOT REACHED`. Older M3.7 preparation checkpoint statements
...
are not WU5 blockers; video remains outside the current M3.8 contract. WU6 is
the next preparation/audit target on this branch; its implementation is not
started and remains unauthorized. Full M3.8 is `NRP NOT REACHED`. The contract is
