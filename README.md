...
MR.1 Installation Refinement: PROMOTED / IMPLEMENTATION IN PROGRESS at `docs/36_mr_1_installation_refinement_contract.md`; the authoritative six-WU staged-installation topology has WU1 Installer Shell, Requirements & Navigation Framework COMPLETE AND CLOSED with technical validation PASS and human acceptance PASS; WU2 Staged Installation Plan & Database Decision is NOT STARTED / NEXT ACTIVE IMPLEMENTATION TARGET; WU3 Administrator & Site Staging, WU4 Optional Core Module Selection, WU5 Final Review, Installation Commit & Result, and WU6 Cross-Step Responsive, Accessibility & Human Acceptance remain NOT STARTED. Review & Install is the first COPOT schema/Admin/Site/optional-Module mutation boundary; database-container provisioning at Database is the explicit capable-environment exception. The Pre-MR.1 Correctness Gate is SATISFIED / VALIDATED: Blocker A `DB_NAMESPACE` persistence is RESOLVED / VALIDATED and Blocker B Change Database / Finalize progression state is CLASSIFIED / NOT A DEFECT. The Cross-Fileset Upgrade Ownership Proof Gap is a separate ARCHITECTURE / PRODUCT INVESTIGATION REQUIRED item and non-blocking by default.
