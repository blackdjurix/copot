...
M2.3 Minimal Site Capabilities is complete and released as v0.11.0. M2.4 Platform Hardening, Post-M2 Distribution & Release Preparation, and the package reproducibility correction are complete and released as v0.12.0. M3 Preparation and M3.1 Users & Access are complete; M3.1 merged to `main` through `5c4cf8c`. Post-M3.1 Roadmap Sync and M3.2 preparation are complete. M3.2 Batches 1–4 are complete; Batch 5 has not started and M3.2 remains in progress.
...
M3.2 Settings Manager is the active milestone in the approved sequence. Exact scope, permission reuse, Core approval points, five-batch plan, acceptance criteria, and manual verification remain authoritative in `docs/17_m3_2_settings_manager_contract.md`. Batch 1 is complete and its no-return gate is reached: the manager owns the configured-path Admin Settings route/view boundary, and `SettingsService` exposes registered-only deterministic read-only discovery, with 42 focused assertions passing. Batch 2 Manager Domain & Field Contract is complete with 94 focused assertions: module-local deterministic section/typed-field contracts expose only approved registered scalar definitions, exclude generic JSON and Site Asset descriptors, aggregate validation before writes, preserve optional omissions, and provide atomic root/nested save behavior. Batch 3 Admin Routes and Presentation is complete with 85 focused assertions: grouped dynamic fields use `SettingsManager::sections()`, nested identifier payloads delegate to `SettingsManager::save()`, validation redisplay and controlled failures remain in the Admin shell, configured paths and PRG remain active, and Logo/Favicon workflows stay specialized. Batch 4 Security and Compatibility Hardening is complete with 145 focused assertions: tests-only coverage proves the two-permission boundary and authorization ordering, missing/invalid CSRF and validation containment, secret/uneditable exclusion, sanitized mid-write rollback, and configured-path plus specialized Site Asset compatibility. Focused M3.2 coverage through Batch 4 totals 366 assertions; relevant M2.1, M2.3, and M3.1 regressions pass. Batch 5 is next but has not started, M3.2 remains in progress, the approved M3 sequence is unchanged, and no new Core touchpoint was introduced.
