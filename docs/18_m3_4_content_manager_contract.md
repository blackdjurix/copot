...
M3.4 Content Manager preparation is locked on the dedicated preparation branch. Implementation, schema changes, permission provisioning, runtime validation, browser validation, commit, push, merge, release, and publication have not started.
...
`content.read` provisioning is a required M3.4 decision and acceptance boundary, but its fresh-install and existing-install implementation remains behind the relevant batch approval gate.
