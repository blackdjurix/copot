# Webcore Product Completeness & Stabilization Contract
...
This contract promotes the reviewed
`precontracts/webcore_product_completeness_stabilization_precontract.md` into
the authoritative repository scope for the corrective Post-M3 Webcore Product
Completeness & Stabilization workstream.

Promotion locks the accepted workstream boundary. It does not by itself
authorize source, runtime, test, schema, configuration, package, or lifecycle
implementation. Each technical execution slice remains subject to explicit
executor-facing authorization under current project governance.

This workstream is not MR.2, is not a new MR milestone, and is not a release
milestone. It is a HARD predecessor to Bundled Module Refinement Preparation &
Reconciliation.

## Objective

Complete the already-accepted Webcore product baseline so that a valid COPOT
installation remains usable as both a site and Admin product with:

- zero optional Modules; and
- zero Themes.

The accepted architecture already places minimum viability in Webcore. This
workstream corrects missing, fragmented, duplicated, or incomplete
product-facing operator projections over already-authoritative Webcore
...
fragmented Branding/Appearance delivery.

## Authoritative baseline

Webcore minimum viability remains authoritative under the closed Post-M3
architecture reconciliation. Optional Modules may extend Webcore but must not
become prerequisites for baseline operation.
...
Existing singular authorities must be preserved:

- Settings definitions, defaults, validation, typed serialization,
  effective-value fallback, and persistence remain under Settings Platform;
- Logo and Favicon validation, storage, activation, cleanup, and public serving
  remain under Site Asset authority;
- Webcore Branding remains the upstream durable authority for accepted baseline
  Branding data and palette behavior;
- Core Media services and persistence remain authoritative for baseline Media;
- Core Navigation services and persistence remain authoritative for the primary
  Navigation baseline.

No competing state, persistence, lifecycle, provider, or ownership authority
may be created merely to support a new product-facing projection.

## Baseline capability and product-projection classification
...
“Webcore Product Completeness” is a scope-control classification. It must not be
interpreted as permission to reopen every Webcore capability.

## Root completeness boundaries

### Core Media Admin

Webcore already owns baseline Media capability. The missing product capability
is an always-available Webcore-owned Admin projection over that existing Core
Media authority.
...
Webcore already owns one usable primary Navigation capability. The missing
product capability is an always-available Webcore-owned Admin management
projection over that authority.

Navigation Manager remains a retained Bundled Module that **EXTENDS** Webcore
Navigation with advanced multi-menu, Theme-location, assignment, provider,
visibility, targeting, and richer management capability.

### Webcore Site Settings

Webcore already has the settings, site identity, asset, localization, and
branding authorities required for baseline site configuration.

The corrective need is one coherent product-facing Site Settings projection
that consolidates operator experience without consolidating or replacing
...
## Site Settings ownership and projection model
...
WU4 may, only after separate technical execution authorization, reconcile the
currently accepted System Manager and historical Settings product-facing
projections into a canonical Webcore Site Settings operator path for Site
Identity, Localization, and bounded Appearance.

This is projection-level supersession only. It does not transfer Settings, Site
Asset, or Branding state, persistence, validation, security, or lifecycle
authority.

Existing System Manager or historical Settings projections may remain only when
source inspection proves a distinct lifecycle-specific, contextual,
compatibility, or recovery purpose. They must not remain competing canonical
editors for the same baseline setting family.

Where WU4 supersedes a product-facing delivery location established by
`docs/39_mr_2_wu2_webcore_system_manager_baseline_contract.md` or described by
`docs/11_branding_foundation.md`, the WU4 contract and closure documentation
must explicitly record that projection-level supersession. Historical accepted
authority records remain historical evidence and must not be rewritten to imply
that the earlier delivery location never existed.
...
Existing setting definitions, validation rules, and specialized asset lifecycle
authorities remain authoritative.

### Localization

Site Settings may expose the accepted registered Localization controls:
...
The target is one canonical product-facing operator path. Existing contextual
or lifecycle-specific projections may remain only when source evidence proves a
distinct justified purpose. Overlapping controls must not be removed blindly.

This workstream does not introduce Admin UI translation, multilingual Content,
Language Packs, new localization architecture, or per-user localization
preferences.

### Appearance

Site Settings may expose the accepted Webcore Branding baseline through a
bounded Appearance projection.

The workstream must not create an arbitrary token editor, editable semantic
role mapping, generic color engine, competing appearance persistence, or turn
Theme-specific advanced appearance into Webcore baseline configuration.

## Site Color Scheme boundary

Site Color Scheme is a bounded site-level resolved projection over existing
Webcore Branding data.

The accepted planning relationship is:
...
Existing Branding data remains upstream durable authority. WU4 may materialize
exact projection and resolution mechanics only within this boundary.
...
Theme may consume the accepted site-level appearance contract and may provide
active-Theme-scoped appearance overrides where separately supported. Such
overrides must not write back to or take ownership of Webcore site-level
Branding data.

Semantic operational colors remain separate, including warning, danger,
success, information, validation, error, destructive-action, and other
accessibility-critical states.

Advanced Brand Kit, multi-brand, white-label, per-user Admin Appearance, and
unrelated Theme customization remain outside this workstream.

## Preserved negative findings

Unless concrete new evidence contradicts them, the following findings remain
locked:
...
- existing Settings, Branding, Site Asset, Media, Navigation, and other Core
  authorities must be preserved;
...
complete for their accepted scopes. WU4 is COMPLETE / ACCEPTED / CLOSED: Batch
1 is implemented and accepted, the Batch 2 System and Modules slices are
...
Purpose: lock the corrective boundary against accepted architecture and current
...
Purpose: provide an always-available Webcore-owned Media Admin projection over
existing Core Media authority.

Subject to exact current source evidence, baseline capability is limited to:
...
Metadata editing is not automatically part of baseline scope. It may be included
only where existing Core Media authority and source evidence already support it
without importing Media Manager semantics.

Search, filters, pagination, card systems, advanced preview, and other workspace
conveniences are not automatic baseline requirements. They may be included only
when exact source/runtime evidence proves a practical baseline need without
turning Core Media Admin into a miniature Media Manager.

Media Manager extension-only behavior remains outside baseline, including
processing variants, derivative management, crop, resize, rotate, advanced
image editing, richer processing workflows, richer workspace conveniences,
folders, galleries, bulk workflows, and advanced organization.

Exact UI mechanics remain WU2 contract work.

### WU3 — Core Primary Navigation Admin Baseline

Purpose: provide an always-available Webcore-owned management projection for
the accepted primary Navigation baseline.
...
- provider browsing or advanced provider capability;
...
Purpose: provide one coherent Webcore Site Settings operator projection while
preserving existing Settings, Site Asset, and Branding authority.

Current state: WU4 is COMPLETE / ACCEPTED / CLOSED. Batch 1 is IMPLEMENTED / ACCEPTED.
...
technical validation, and human/product acceptance. Batch 4 is COMPLETE /
ACCEPTED / CLOSED after technical and human/product acceptance; the WU5
Webcore Content Product Completeness corrective unit is COMPLETE / ACCEPTED /
CLOSED.
...
The WU may organize controls into Site Identity, Localization, and Appearance.
It must reconcile duplicated, overlapping, and fragmented product-facing
surfaces.

The target is one canonical Site Settings operator path. WU4 may supersede
System Manager or historical Settings as the canonical product-facing delivery
location for an affected baseline setting family, but only at the projection
layer. It must not supersede or duplicate underlying Settings, Site Asset, or
Branding authority.

A contextual, compatibility, recovery, or lifecycle-specific projection may
remain when source inspection proves a distinct justified function. Any
retained projection must not remain a competing canonical editor for the same
baseline setting family.

WU4 must:

- preserve Settings authority;
- preserve Site Asset authority;
- preserve Branding authority;
- explicitly record projection-level supersession of previously accepted System
  Manager or historical Settings delivery locations;
- avoid competing persistence;
- avoid new provider arbitration;
- avoid generic settings-editor behavior; and
- avoid wholesale System Manager reopening.

Site Color Scheme may be materialized only as a bounded resolved projection over
accepted Branding lineage.
...
With zero optional Modules and zero Themes, acceptance must demonstrate that:
...
- primary Navigation is operable through Core authority and Core Admin
  projection;
- Site Settings are operable without an optional or retired product-facing
  Settings Manager;
- Built-in Public View remains usable;
- required Site Identity remains operable;
- supported Appearance behavior remains available; and
...
WU6 is COMPLETE / ACCEPTED / CLOSED with technical acceptance PASS and
human/product acceptance NOT REQUIRED. The accepted zero-optional baseline
proved zero enabled optional Modules and zero Themes, with integrated viability
across Core Content, Core Media, Core Primary Navigation, Site Settings, Site
Identity, supported Appearance, Homepage composition, and Built-in Public View.
Retained Content Manager, Media Manager, and Navigation Manager coexistence was
proven as extension behavior without replacement, forking, or takeover of
Webcore authority. No accepted-baseline regression or extension-boundary
violation was found. The focused Core Media stale assertion was corrected and
validated in `b8e7b39baec3359d7f1b33704d35a22696df172c`.
...
This workstream is a HARD predecessor to Bundled Module Refinement Preparation &
Reconciliation.
...
This sequence is planning topology only and does not authorize downstream
implementation.

The HARD dependency exists because retained Bundled Modules must be refined
against a complete zero-optional Webcore baseline, not against product gaps that
force optional Modules to behave as accidental prerequisites.

## Acceptance model

### Objective / AI-verifiable acceptance
...
- deterministic accessibility criteria;
- responsive and technical presentation checks;
- directly impacted regressions; and
- final diff and unintended-change review.

### Human/product acceptance

Human/product review is required only where subjective judgment is materially
necessary. Expected gates include:
...
Implementation Work Units must use a freshly verified runtime derived from
authoritative implementation source.

The historical `C:\xampp\htdocs\copot.test` runtime must not be treated as
current authority without fresh verification.

Validation must begin closest to each change and expand only according to actual
impact. Broad historical suites must not be rerun merely for ceremony when
accepted evidence remains valid and no regression signal exists.

Zero-optional acceptance must prove the intended baseline with optional Modules
and Themes absent, disabled, or otherwise non-required according to the exact
acceptance contract.

Human/product review should use a disposable or safely controlled runtime where
required and must not silently mutate an unrelated authoritative development
instance.
...
- package retirement merely because an old product-facing Manager is no longer
  preferred;
- destructive cleanup;
...
## v0.14.0 readiness boundary

Successful closure of this workstream is intended to establish a
product-completeness and stabilization boundary suitable for future v0.14.0
planning.

The intended post-condition is:

- Webcore minimum viability is product-operable as well as architecturally
  owned;
- zero optional Modules and zero Themes remain a valid baseline;
- major Webcore surgery becomes exceptional rather than a normal prerequisite
  of Bundled Module refinement; and
- later refinement may still introduce bounded corrections or extension seams
  when justified.

This does not mean v0.14.0 must be released immediately after workstream
closure. Release planning, version advancement, packaging, tag, publication,
and distribution remain separate explicit gates.
...
Do not infer technical implementation authorization from:

- Workplan state;
- this contract's promotion state;
- the Pre-contract;
- Handoff state;
- NRP state;
- previous workstream closure; or
- future release intent.

No Deferred Item, destructive action, production reconciliation, release, tag,
publication, or external distribution is adopted or authorized here.
...
Primary authoritative and supporting sources:
...
- `workplan.md` for planning/provenance indexing only.

Existing source code and tests remain implementation evidence. They must be
inspected progressively per Work Unit rather than treated as implicit expansion
of this contract.
