...
$app->router()->get($mediaPath, function ($request) use ($app, $mediaAdmin, $mediaRepository, $mediaUsages, $requireMedia, $render, $mediaPath): Response {
    ...
    $bar = $user->can('media.upload') ? '<a class="admin-button admin-button--primary" href="' . htmlspecialchars($app->adminUrl()->childUrl('media/upload'), ENT_QUOTES, 'UTF-8') . '">Upload</a>' : null;
    return $render('list', ['items' => $items, 'evidence' => $evidence, 'canDelete' => $user->can('media.delete'), 'canUpload' => $user->can('media.upload'), 'csrfToken' => $app->csrf()->token(), 'notice' => $request->input('notice'), 'error' => $request->input('error'), 'adminUrl' => fn (string $path = '') => $app->adminUrl()->childUrl($path), 'mediaUrl' => fn (int $id): string => $app->url('/media/' . $id)], $user, $request->path(), 200, ['description' => 'Manage uploaded media files.', 'bar' => $bar, 'surface' => 'transparent', 'spacing' => 'default']);
});
