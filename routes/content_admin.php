...
$renderForm = static function (string $title, string $action, array $data, array $errors, $user, string $path, string $mode) use ($app, $contentRoute, $mediaRepository): Response {
    ...
    $canUploadMedia = $user->can('media.upload');
    ...
    $html .= '<div class="admin-field"><label class="admin-field__label" for="body">Body</label><textarea id="body" name="body" rows="12" required>' . htmlspecialchars((string) $data['body'], ENT_QUOTES, 'UTF-8') . '</textarea></div></fieldset>';
    $html .= '<aside class="admin-content-form-sidebar"><fieldset class="admin-fieldset" data-core-content-media-picker data-picker-url="' . htmlspecialchars($app->adminUrl()->childUrl('media/select'), ENT_QUOTES, 'UTF-8') . '" data-upload-url="' . htmlspecialchars($app->adminUrl()->childUrl('media/select/upload'), ENT_QUOTES, 'UTF-8') . '" data-csrf-token="' . htmlspecialchars($app->session()->csrfToken(), ENT_QUOTES, 'UTF-8') . '" data-can-upload="' . ($canUploadMedia ? '1' : '0') . '" data-selected-media="' . $featuredJson . '"><legend>Featured Image</legend><input id="featured_media_id" name="featured_media_id" type="hidden" value="' . htmlspecialchars($featuredId === null ? '' : (string) $featuredId, ENT_QUOTES, 'UTF-8') . '" data-core-content-media-input><p class="admin-field__help" data-core-content-media-status aria-live="polite">PNG, JPG, or WebP. Maximum 2 MB; maximum 4096 × 4096 px.</p><div class="admin-media-picker__selected" data-core-content-media-selected' . ($featuredDescriptor === null ? ' hidden' : '') . '></div><div class="admin-content-featured-placeholder" data-core-content-media-placeholder' . ($featuredDescriptor !== null ? ' hidden' : '') . ' aria-hidden="true"></div><div class="admin-actions"><button class="admin-button admin-button--secondary" type="button" data-core-content-media-open' . (!$canUseMedia ? ' disabled' : '') . '>' . ($featuredDescriptor === null ? 'Select media' : 'Change') . '</button><button class="admin-button admin-button--link" type="button" data-core-content-media-clear' . ($featuredDescriptor === null || !$canUseMedia ? ' hidden' : '') . '>Clear</button></div>' . (!$canUseMedia ? '<p class="admin-field__help">Media selection requires the Media use permission.</p>' : '') . '<dialog class="admin-media-picker core-content-media-picker" data-core-content-media-dialog aria-labelledby="core-content-media-picker-title"><div class="admin-media-picker__panel"><h3 id="core-content-media-picker-title">Select Featured Media</h3><p class="admin-field__help">Choose an image from Core Media.</p><div class="admin-field core-content-media-picker__search"><label class="admin-field__label" for="core-content-media-search">Search media</label><input id="core-content-media-search" type="search" data-core-content-media-search aria-controls="core-content-media-results" placeholder="Search title or filename" autocomplete="off"></div><div class="admin-media-picker__results" data-core-content-media-results aria-live="polite"></div>' . ($canUploadMedia ? '<div class="core-content-media-picker__upload"><div class="admin-field"><label class="admin-field__label" for="core-content-media-upload">Upload New</label><input id="core-content-media-upload" type="file" accept="image/jpeg,image/png,image/webp" data-core-content-media-upload></div><div class="admin-actions"><button class="admin-button admin-button--secondary" type="button" data-core-content-media-close>Cancel</button><button class="admin-button admin-button--primary" type="button" data-core-content-media-upload-button>Upload</button></div></div>' : '<div class="admin-actions core-content-media-picker__actions"><button class="admin-button admin-button--secondary" type="button" data-core-content-media-close>Cancel</button></div>') . '</div></dialog></fieldset></aside></div>';
    $html .= '<script src="' . htmlspecialchars($app->url('/admin-assets/js/core-content-featured-media.js?v=wu5-1'), ENT_QUOTES, 'UTF-8') . '" defer></script></form>';
    $html .= '<div class="admin-content-form-action-bar"><div class="admin-content-form-action-bar__group admin-content-form-action-bar__group--form"><a class="admin-button admin-button--secondary" href="' . htmlspecialchars($contentRoute(), ENT_QUOTES, 'UTF-8') . '">Cancel</a><button class="admin-button admin-button--primary" type="submit" form="core-content-form">' . ($mode === 'create' ? 'Create content' : 'Save changes') . '</button></div>';
    if (($data['id'] ?? null) !== null) {
        $html .= '<div class="admin-content-form-action-bar__group admin-content-form-lifecycle" aria-label="Content lifecycle actions">';
...
$app->router()->get($app->adminUrl()->routeChildUrl('content'), function ($request) use ($app, $contentRepository, $mediaRepository, $requireContent, $contentRoute, $authorLabels): Response {
    $user = $requireContent($request, 'content.read');
    if ($user instanceof Response) return $user;
    $workspace = $contentRepository->paginate(25, 0);
    $html = '<section class="admin-content-page admin-stack"><div class="admin-panel admin-content-filters admin-filter-toolbar" data-core-content-list-filters aria-label="Content filters"><div class="admin-content-filter-field admin-content-search"><label for="core-content-list-search">Search</label><input id="core-content-list-search" type="search" data-core-content-list-search aria-controls="core-content-list-table" placeholder="Title or slug" autocomplete="off"></div><div class="admin-content-filter-field"><label for="core-content-list-type">Type</label><select id="core-content-list-type" data-core-content-list-type><option value="">All</option><option value="page">Page</option><option value="article">Article</option></select></div><div class="admin-content-filter-field"><label for="core-content-list-status">Status</label><select id="core-content-list-status" data-core-content-list-status><option value="">All</option><option value="draft">Draft</option><option value="published">Published</option><option value="archived">Archived</option></select></div><div class="admin-content-filter-field"><label for="core-content-list-page-size">Per page</label><select id="core-content-list-page-size" data-core-content-list-page-size><option value="25">25</option><option value="50">50</option><option value="100">100</option></select></div><div class="admin-content-filter-actions"><button class="admin-button admin-button--secondary" type="button" data-core-content-list-apply>Apply Filter</button></div></div><div class="admin-panel admin-content-table-panel"><div class="admin-panel__header admin-content-table-panel__header"><div class="admin-panel__heading"><h2 class="admin-panel__title">Content Entries</h2></div>';
    ...
    $html .= '</div><div class="admin-panel__body">';
    if ($workspace === []) {
        ...
        foreach ($workspace as $item) {
            ...
            $featured = $item->featuredMediaId() === null ? null : $mediaRepository->findById((int) $item->featuredMediaId());
            $thumbnail = $featured !== null && $featured->kind() === 'image'
                ? '<img class="admin-content-thumbnail" src="' . htmlspecialchars($app->url('/media/' . $featured->id()->value()), ENT_QUOTES, 'UTF-8') . '" alt="">'
                : '<span class="admin-content-thumbnail admin-content-thumbnail--empty" aria-hidden="true"></span>';
            $html .= '<tr data-core-content-row data-content-title="' . htmlspecialchars(strtolower($item->title()), ENT_QUOTES, 'UTF-8') . '" data-content-slug="' . htmlspecialchars(strtolower($item->slug()), ENT_QUOTES, 'UTF-8') . '" data-content-type="' . htmlspecialchars($item->type(), ENT_QUOTES, 'UTF-8') . '" data-content-status="' . htmlspecialchars($item->status(), ENT_QUOTES, 'UTF-8') . '"' . $rowAttributes . '><td data-label="Thumbnail">' . $thumbnail . '</td><td data-label="Title"><div class="admin-content-title-cell"><strong>' . htmlspecialchars($item->title(), ENT_QUOTES, 'UTF-8') . '</strong><small>' . htmlspecialchars($item->slug(), ENT_QUOTES, 'UTF-8') . '</small></div></td><td data-label="Type">' . htmlspecialchars(ucfirst($item->type()), ENT_QUOTES, 'UTF-8') . '</td><td data-label="Status">' . htmlspecialchars(ucfirst($item->status()), ENT_QUOTES, 'UTF-8') . '</td><td data-label="Updated">' . htmlspecialchars((string) $item->updatedAt(), ENT_QUOTES, 'UTF-8') . '</td><td data-label="Author">' . htmlspecialchars($authorLabel, ENT_QUOTES, 'UTF-8') . '</td></tr>';
        }
        $html .= '</tbody></table></div><p class="admin-empty-state admin-content-list-no-results" data-core-content-list-empty hidden role="status">No Content matches your filters.</p><div class="admin-content-table-panel__footer">' . ($user->can('content.create') ? '<a class="admin-button admin-button--primary" href="' . htmlspecialchars($contentRoute('create'), ENT_QUOTES, 'UTF-8') . '">Create Content</a>' : '') . '</div>';
    }
    $html .= '</div></div><script src="' . htmlspecialchars($app->url('/admin-assets/js/core-content-list.js?v=wu5-3'), ENT_QUOTES, 'UTF-8') . '" defer></script></section>';
    return Response::html($app->adminPageRenderer()->render('Content', $html, $user, $app->session()->csrfToken(), $request->path(), ['description' => 'Webcore Pages and Articles.', 'surface' => 'transparent', 'spacing' => 'default'], []));
});
