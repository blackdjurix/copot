...
$renderForm = static function (string $title, string $action, array $data, array $errors, $user, string $path, string $mode) use ($app, $contentRoute, $mediaRepository): Response {
    ...
    $html .= '<aside class="admin-content-form-sidebar"><fieldset class="admin-fieldset" data-core-content-media-picker data-picker-url="' . htmlspecialchars($app->adminUrl()->childUrl('media/select'), ENT_QUOTES, 'UTF-8') . '" data-selected-media="' . $featuredJson . '"><legend>Featured Image</legend><input id="featured_media_id" name="featured_media_id" type="hidden" value="' . htmlspecialchars($featuredId === null ? '' : (string) $featuredId, ENT_QUOTES, 'UTF-8') . '" data-core-content-media-input><p class="admin-field__help" data-core-content-media-status aria-live="polite">PNG, JPG, or WebP. Maximum 2 MB; maximum 4096 × 4096 px.</p><div class="admin-media-picker__selected" data-core-content-media-selected' . ($featuredDescriptor === null ? ' hidden' : '') . '></div><div class="admin-content-featured-placeholder" data-core-content-media-placeholder' . ($featuredDescriptor !== null ? ' hidden' : '') . ' aria-hidden="true"></div><div class="admin-actions"><button class="admin-button admin-button--secondary" type="button" data-core-content-media-open' . (!$canUseMedia ? ' disabled' : '') . '>' . ($featuredDescriptor === null ? 'Select media' : 'Change') . '</button><button class="admin-button admin-button--link" type="button" data-core-content-media-clear' . ($featuredDescriptor === null || !$canUseMedia ? ' hidden' : '') . '>Clear</button></div>' . (!$canUseMedia ? '<p class="admin-field__help">Media selection requires the Media use permission.</p>' : '') . '<dialog class="admin-media-picker core-content-media-picker" data-core-content-media-dialog aria-labelledby="core-content-media-picker-title"><div class="admin-media-picker__panel"><h3 id="core-content-media-picker-title">Select Featured Media</h3><p class="admin-field__help">Choose an image from Core Media.</p><div class="admin-field"><label class="admin-field__label" for="core-content-media-search">Search media</label><input id="core-content-media-search" type="search" data-core-content-media-search aria-controls="core-content-media-results" placeholder="Search title or filename" autocomplete="off"></div><div class="admin-media-picker__results" data-core-content-media-results aria-live="polite"></div><div class="admin-actions"><button class="admin-button admin-button--secondary" type="button" data-core-content-media-close>Cancel</button></div></div></dialog></fieldset></aside></div>';
