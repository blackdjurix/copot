...
$app->router()->post($path, function ($request) use ($app, $requireUser, $render, $hero, $path): Response {
    ...
    try { $hero()->set($heroId); foreach ($submitted as $identifier => $candidate) { [$namespace, $key] = explode('.', $identifier, 2); $app->settings()->set($namespace, $key, $candidate, 'string'); } $connection->commit();
