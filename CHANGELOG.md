...
- Completed M3.6 Navigation Manager Work Unit 2 — Navigation Domain Service and Hierarchy on `feature/m3.6-navigation-manager` at `7e54d0309c8ca75f0f32200394fd2a271c8a9b83` (`feat(m3.6): add navigation domain hierarchy service`). Menu/item value objects, the repository, and domain service now enforce five levels including root, same-menu parenting, self/descendant/cycle rejection, moved-subtree depth validation, deterministic ordering, exact reorder, target payload invariants, accepted custom-URL grammar, and standalone/caller-owned transaction isolation. Focused WU2 validation passed 44 assertions; direct WU1 Navigation, Content, and M3.5 Taxonomy regressions passed. M3.6 preparation is `NRP CONFIRMED`; WU1 and WU2 are `NRP CONFIRMED`; WU3 is next and not started; full M3.6 is `NRP NOT REACHED`. Content resolver implementation, production Taxonomy targets, Theme location declaration/consumption, and the Admin workspace remain deferred.
