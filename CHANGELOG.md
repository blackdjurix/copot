...
- Completed and human-accepted MR.2 WU2 Webcore System Manager Baseline on its
  feature branch, preserving existing lifecycle, Settings, Site Asset, Module,
  and System Health authority while delivering the Webcore-only fallback
  surface.
...
- Preserved IU2 as not started and separately authorized; Module Package
  Lifecycle final human/E2E acceptance is now PASS and closed on authoritative
  `main`.
...
- Preserved release-based advancement: completed features do not create
  ...
  release event.
- Production Webcore reconciliation remains NOT STARTED and is not part of the
  reconciled `0.13.0` contents.
...
- Added `docs/23_m3_7_theme_manager_contract.md`, locking the first-party `theme-manager` Admin boundary over the existing Core Theme System, controlled declarative theme settings, active-theme and activation safety, Navigation and branding boundaries, `admin.access`/`themes.manage`, exclusions, design checkpoint, and exactly six M3.7 preparation work units. M3.7 preparation is independent after M3.6 closure; full M3.7 is not complete.

### Changed

- Reconciled M3.6 WU1–WU6, full Navigation Manager, integration, and feature-branch lifecycle as closed; synchronized roadmap, project instructions, README, Theme, Settings, and Navigation documentation.

- Completed M3.6 Navigation Manager Work Unit 6 security closure at `62a2dfb892f06005644c82cba1015ed5dc71b0e7` (`test(m3.6): close navigation security gate`). The focused security-closure test passed 15 assertions covering bounded frontend context paths, fail-closed contributor registration and composition, safe diagnostics, request/render isolation, deterministic context collisions, and representative Admin payload guards. The final feature-branch documentation correction is `8deb4f67d47f476e254b36e3802bd250e393921d` (`docs(m3.6): correct WU6 security closure anchor`). M3.6 was fast-forward integrated into `main` with no merge commit; the feature branch lifecycle is closed. M3.6 WU1–WU6 and full M3.6 are complete. `AdminNavigation` remains separate and production Taxonomy targets remain deferred. M3.7 Theme Manager preparation is the active independent documentation target; full M3.7 is not complete. Release, tag, and publication have not occurred.

- Historical M3.6 WU4 checkpoint: the Admin Management Workspace was implemented on `feature/m3.6-navigation-manager` at `43085441a82ecc38941ca880b2dbbd6823f61215` (`feat(m3.6): add navigation admin workspace`). The checkpoint recorded WU4 as a candidate before later WU5/WU6 completion and final M3.6 reconciliation.

- Completed M3.6 Navigation Manager Work Unit 3 — Content Target Resolver Integration on `feature/m3.6-navigation-manager` at `3e387c5756636d08416880a8d8ec5ca1112cad75` (`feat(m3.6): add content navigation target resolver`). Content owns `ContentNavigationTargetResolver`; Navigation owns request-scoped registry creation and optional-provider composition. The `content` provider accepts canonical lowercase globally unique Content slugs, resolves published Content only to `/content/{slug}` with the stored title, reflects title/slug/publication changes without cross-request caching, and fails closed for missing, stale, malformed, draft, archived, disabled, and unavailable targets. No Core wiring, global registry, or service locator was introduced. Focused WU3 resolver validation passed 19 assertions; direct WU2, WU1, and Content public publication/security regressions passed. M3.6 preparation and WU1–WU3 are complete; WU4 is next and not started; full M3.6 is not complete. Navigation item visibility and Theme rendering remain deferred to WU5, the Admin workspace to WU4, and production Taxonomy targets remain deferred.

- Completed M3.6 Navigation Manager Work Unit 2 — Navigation Domain Service and Hierarchy on `feature/m3.6-navigation-manager` at `7e54d0309c8ca75f0f32200394fd2a271c8a9b83` (`feat(m3.6): add navigation domain hierarchy service`). Menu/item value objects, the repository, and domain service now enforce five levels including root, same-menu parenting, self/descendant/cycle rejection, moved-subtree depth validation, deterministic ordering, exact reorder, target payload invariants, accepted custom-URL grammar, and standalone/caller-owned transaction isolation. Focused WU2 validation passed 44 assertions; direct WU1 Navigation, Content, and M3.5 Taxonomy regressions passed. M3.6 preparation is complete; WU1 and WU2 are complete; full M3.6 is not complete. At WU2 delivery, WU3 was the next unstarted gate; Content resolver implementation, production Taxonomy targets, Theme location declaration/consumption, and the Admin workspace remained deferred.

- Completed M3.6 Navigation Manager Work Unit 1 — Contract, Provisioning, and Compatibility Foundation on `feature/m3.6-navigation-manager` at `38d75fa1d676371b5c78e392871649a663e573db` (`feat(m3.6): establish navigation WU1 foundation`). The Navigation manifest, provider-neutral resolver contracts, baseline and idempotent existing-install provisioning, `navigation.manage`, baseline ModuleManager activation, package inclusion, and clean-install expectations are in place. Focused WU1 compatibility passed 23 assertions; direct Module Manager, Content, Taxonomy, package, distribution, clean-install, lint, and diff checks passed. M3.6 preparation is complete; WU1 is complete; full M3.6 is not complete. Content resolver implementation, production Taxonomy targets, Theme location declaration/consumption, Admin workspace, and hierarchy behavior were deferred from WU1.

- Added the M3.6 Navigation Manager preparation contract at `docs/22_m3_6_navigation_manager_contract.md`. At preparation acceptance, M3.6 preparation was closure pending; full M3.6 was not complete and implementation had not started. The documentation locks Navigation ownership, Theme-declared locations, optional Content target resolution, deferred production Taxonomy targets, separate `AdminNavigation`, no currently justified Core change, and six responsibility-level work units.

- Closed M3.5 Taxonomy Manager Work Unit 5 and fast-forward integrated the completed feature branch into `main` at `b09a01ca0a93cfa2eb9eccafd648f7a708df1576`, with no merge commit. Focused Taxonomy, Content compatibility, package, clean-install, PHP lint, browser, accessibility, documentation, Git, containment, and branch-lifecycle verification passed; WU1–WU5 and full M3.5 are complete. Local and remote feature branches are deleted. M3.6 is next and not started; release, tag, and publication remain not started and separately authorized.
...
- Integrated the completed Admin UX Refinement 1 and Shell Foundation work into `main` in parent-first order with `--ff-only` at `69fda0d`; both feature branches were subsequently deleted locally and remotely after verified containment, and no PR was used.
