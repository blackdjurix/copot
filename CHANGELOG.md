...
- Completed M3.6 Navigation Manager Work Unit 3 — Content Target Resolver Integration on `feature/m3.6-navigation-manager` at `3e387c5756636d08416880a8d8ec5ca1112cad75` (`feat(m3.6): add content navigation target resolver`). Content owns `ContentNavigationTargetResolver`; Navigation owns request-scoped registry creation and optional-provider composition. The `content` provider accepts canonical lowercase globally unique Content slugs, resolves published Content only to `/content/{slug}` with the stored title, reflects title/slug/publication changes without cross-request caching, and fails closed for missing, stale, malformed, draft, archived, disabled, and unavailable targets. No Core wiring, global registry, or service locator was introduced. Focused WU3 resolver validation passed 19 assertions; direct WU2, WU1, and Content public publication/security regressions passed. M3.6 preparation, WU1, and WU2 are `NRP CONFIRMED`; WU3 is `NRP CANDIDATE`; WU4 is next and not started; full M3.6 is `NRP NOT REACHED`. Navigation item visibility and Theme rendering remain deferred to WU5, the Admin workspace to WU4, and production Taxonomy targets remain deferred.
