...
- Completed M3.6 Navigation Manager Work Unit 6 security closure on the feature branch at `62a2dfb892f06005644c82cba1015ed5dc71b0e7` (`test(m3.6): close navigation security gate`). The focused security-closure test passed 15 assertions covering bounded frontend context paths, fail-closed contributor registration and composition, safe diagnostics, request/render isolation, deterministic context collisions, and representative Admin payload guards. Full M3.6 implementation, validation, runtime/browser review, human acceptance, security closure, and feature-branch documentation are complete; WU1–WU5 are `NRP CONFIRMED`, while full M3.6 remains `NRP NOT REACHED` pending separately authorized fast-forward integration and branch lifecycle closure. `AdminNavigation` remains separate, production Taxonomy targets and Theme Manager remain deferred, and merge, release, tag, and publication have not occurred.

- Completed M3.6 Navigation Manager Work Unit 4 — Admin Management Workspace on `feature/m3.6-navigation-manager` at `43085441a82ecc38941ca880b2dbbd6823f61215` (`feat(m3.6): add navigation admin workspace`). The Navigation Admin workspace supports menu and item management, custom and provider targets, hierarchy, parent exclusion, exact sibling reorder, empty and unavailable-provider states, permission handling, CSRF, validation, PRG, configured Admin paths, active Admin navigation, and responsive presentation at `1440×900`, `390×844`, `320×800`, and `200%` zoom. Console and network inspection, objective/runtime validation, provisioning and upgrade rerun safety, source/runtime synchronization, and human acceptance passed. `AdminNavigation` remains separate from managed public Navigation data; Theme location declaration/consumption, frontend rendering, design adjustment, and production Taxonomy targets remain deferred to WU5. WU4 is `NRP CANDIDATE`; full M3.6 remains `NRP NOT REACHED`.

- Completed M3.6 Navigation Manager Work Unit 3 — Content Target Resolver Integration on `feature/m3.6-navigation-manager` at `3e387c5756636d08416880a8d8ec5ca1112cad75` (`feat(m3.6): add content navigation target resolver`). Content owns `ContentNavigationTargetResolver`; Navigation owns request-scoped registry creation and optional-provider composition. The `content` provider accepts canonical lowercase globally unique Content slugs, resolves published Content only to `/content/{slug}` with the stored title, reflects title/slug/publication changes without cross-request caching, and fails closed for missing, stale, malformed, draft, archived, disabled, and unavailable targets. No Core wiring, global registry, or service locator was introduced. Focused WU3 resolver validation passed 19 assertions; direct WU2, WU1, and Content public publication/security regressions passed. M3.6 preparation and WU1–WU3 are `NRP CONFIRMED`; WU4 is next and not started; full M3.6 is `NRP NOT REACHED`. Navigation item visibility and Theme rendering remain deferred to WU5, the Admin workspace to WU4, and production Taxonomy targets remain deferred.

- Completed M3.6 Navigation Manager Work Unit 2 — Navigation Domain Service and Hierarchy on `feature/m3.6-navigation-manager` at `7e54d0309c8ca75f0f32200394fd2a271c8a9b83` (`feat(m3.6): add navigation domain hierarchy service`). Menu/item value objects, the repository, and domain service now enforce five levels including root, same-menu parenting, self/descendant/cycle rejection, moved-subtree depth validation, deterministic ordering, exact reorder, target payload invariants, accepted custom-URL grammar, and standalone/caller-owned transaction isolation. Focused WU2 validation passed 44 assertions; direct WU1 Navigation, Content, and M3.5 Taxonomy regressions passed. M3.6 preparation is `NRP CONFIRMED`; WU1 and WU2 are `NRP CONFIRMED`; full M3.6 is `NRP NOT REACHED`. At WU2 delivery, WU3 was the next unstarted gate; Content resolver implementation, production Taxonomy targets, Theme location declaration/consumption, and the Admin workspace remained deferred.

- Completed M3.6 Navigation Manager Work Unit 1 — Contract, Provisioning, and Compatibility Foundation on `feature/m3.6-navigation-manager` at `38d75fa1d676371b5c78e392871649a663e573db` (`feat(m3.6): establish navigation WU1 foundation`). The Navigation manifest, provider-neutral resolver contracts, baseline and idempotent existing-install provisioning, `navigation.manage`, baseline ModuleManager activation, package inclusion, and clean-install expectations are in place. Focused WU1 compatibility passed 23 assertions; direct Module Manager, Content, Taxonomy, package, distribution, clean-install, lint, and diff checks passed. M3.6 preparation is `NRP CONFIRMED`; WU1 is `NRP CONFIRMED`; full M3.6 is `NRP NOT REACHED`. Content resolver implementation, production Taxonomy targets, Theme location declaration/consumption, Admin workspace, and hierarchy behavior were deferred from WU1.

- Added the M3.6 Navigation Manager preparation contract at `docs/22_m3_6_navigation_manager_contract.md`. At preparation acceptance, M3.6 preparation was `NRP CANDIDATE`; full M3.6 was `NRP NOT REACHED` and implementation had not started. The documentation locks Navigation ownership, Theme-declared locations, optional Content target resolution, deferred production Taxonomy targets, separate `AdminNavigation`, no currently justified Core change, and six responsibility-level work units.
