...
final class ModulePackageOperator
{
    ...
    public function enrich(array $items): array
    {
        ...
        foreach ($items as &$item) { $name = (string) ($item['name'] ?? ''); $candidate = $latest[$name] ?? null; $state = null; if ($states instanceof \Copot\Core\ModuleLifecycleStateStore) { try { $state = $states->read($name); if ($state instanceof \Copot\Core\ModuleLifecycleState) { $item['lifecycle_state'] = $state->enabled() ? 'installed_enabled' : 'installed_disabled'; $item['version'] = $state->packageVersion(); $item['lifecycle_managed'] = true; } } catch (\Throwable) { $item['lifecycle_managed'] = false; } } if (is_array($candidate)) { $item['available_package_version'] = $candidate['package_version']; $item['available_package_release'] = $candidate['release_identity']; $item['available_package_candidate'] = $candidate['candidate_key']; if ($state instanceof \Copot\Core\ModuleLifecycleState && \Copot\Core\PackageVersion::compare((string) $candidate['package_version'], $state->packageVersion()) < 0) $item['lifecycle_blocker'] = 'Downgrade and reverse transitions are unsupported.'; elseif ($state === null && ($item['lifecycle_state'] ?? 'not_installed') !== 'not_installed') $item['lifecycle_blocker'] = 'Legacy Module installation requires explicit lifecycle bootstrap.'; } } unset($item);
        foreach ($latest as $name => $candidate) if (!$this->has($items, $name)) $items[] = ['name' => $name, 'title' => $candidate['title'], 'version' => '', 'lifecycle_state' => 'not_installed', 'discovery_state' => 'missing', 'available_package_version' => $candidate['package_version'], 'available_package_release' => $candidate['release_identity'], 'available_package_candidate' => $candidate['candidate_key'], 'available_actions' => [], 'denial_reasons' => []];
    ...
    public function execute(string $candidateKey): string
    {
        ...
        try {
            ...
            $ledger = new ModuleMigrationLedger($storage); $migrations = new ModuleMigrationReconciler($ledger); $permissions = new ModulePermissionReconciler(fn ($module): array => $repo->permissionsFor($module->value()), function ($module, $permission) use ($repo): void { $repo->upsertPermissionMetadata($module->value(), $permission->slug(), $permission->name()); });
            ...
            $result = $coordinator->execute($inspection, $transition, $conflicts, $this->app->database()->connection(), function ($connection, $transition, $inspection) use ($migrations) { $module = $inspection->contract()->moduleIdentity(); if ($transition->classification() === \Copot\Core\ModuleTransitionPlan::INSTALL) return $migrations->freshBaseline($module, 'canonical-current', static function (): void {}); return $migrations->reconcile($connection, $module, $inspection->contract()->packageVersion(), new ModuleMigrationRegistry($module, $inspection->contract()->migrationDeclaration()), (string) ($transition->currentState()?->migrationStateIdentity() ?? ''), null); }, fn ($transition, $inspection) => (new ModuleProvisioningReconciler())->reconcile($inspection->contract()->moduleIdentity(), $inspection->contract()->provisioningDeclaration()), fn ($transition, $inspection) => $permissions->reconcile($inspection->contract()->moduleIdentity(), $inspection->contract()->provisioningDeclaration()));
