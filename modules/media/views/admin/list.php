...
<header class="admin-panel__header">
    <div class="admin-panel__heading"><h2 class="admin-panel__title">Media</h2><p class="admin-panel__description">Browse and manage uploaded images and documents without exposing storage details.</p><p class="admin-media-context">Showing <?= $esc($total) ?> <?= $total === 1 ? 'item' : 'items' ?><?= $hasFilters ? ' matching the current filters' : '' ?>.</p></div>
...
<form class="admin-panel admin-content-filters admin-media-filters admin-filter-toolbar" method="get" action="<?= $esc($adminUrl('media')) ?>" aria-label="Media filters">
    ...
    <span id="media-filter-help" class="admin-media-filters__help">Filters are preserved while you move between pages.</span>
</form>
<?php if ($total === 0): ?>
    <div class="admin-empty-state"><h3 class="admin-empty-state__title"><?= $hasFilters ? 'No matching media' : 'No media yet' ?></h3><p class="admin-empty-state__description"><?= $hasFilters ? 'Try changing the search or filters, or clear them to see the full library.' : 'Upload the first image or document to start the library.' ?></p><?php if ($hasFilters): ?><div class="admin-empty-state__actions"><a class="admin-button admin-button--secondary" href="<?= $esc($adminUrl('media')) ?>">Clear filters</a></div><?php elseif ($canUpload): ?><div class="admin-empty-state__actions"><a class="admin-button admin-button--primary" href="<?= $esc($adminUrl('media/upload')) ?>">Upload media</a></div><?php endif; ?></div>
<?php else: ?>
    <section class="admin-panel admin-media-grid-panel" aria-label="Media items">
        <div class="admin-panel__body">
            <div class="admin-media-grid">
                <?php foreach ($mediaItems as $index => $item): $editable = $isEditable($item); $mediaId = $item->id()->value(); $itemEvidence = $evidence[$mediaId] ?? ['usageCount' => 0, 'variantCount' => 0, 'variantWidths' => []]; ?>
                    <article class="admin-media-card" tabindex="0" role="button" data-media-card data-media-index="<?= $esc($index) ?>" data-media-id="<?= $esc($mediaId) ?>" data-media-title="<?= $esc($item->title()) ?>" data-media-filename="<?= $esc($item->originalFilename()) ?>" data-media-kind="<?= $esc($item->kind()) ?>" data-media-mime="<?= $esc($item->mimeType()) ?>" data-media-bytes="<?= $esc($formatBytes($item->byteSize())) ?>" data-media-width="<?= $esc($item->width() ?? '') ?>" data-media-height="<?= $esc($item->height() ?? '') ?>" data-media-updated="<?= $esc($item->updatedAt()) ?>" data-media-editable="<?= $editable ? '1' : '0' ?>" data-media-usage-count="<?= $esc($itemEvidence['usageCount']) ?>" data-media-variant-count="<?= $esc($itemEvidence['variantCount']) ?>" data-media-variant-widths="<?= $esc(implode(', ', $itemEvidence['variantWidths'])) ?>" data-media-public-url="<?= $esc($url('/media/' . $mediaId)) ?>" data-media-title-url="<?= $esc($adminUrl('media/' . $mediaId . '/title')) ?>" data-media-delete-url="<?= $esc($adminUrl('media/' . $mediaId . '/delete')) ?>" aria-label="Open media <?= $esc($item->title()) ?> details">
                        ...
                        <div class="admin-media-card__body">
                            ...
                            <div class="admin-media-card__status"><span><?= $editable ? 'Editable' : 'Manage-only' ?></span><?php if ($itemEvidence['usageCount'] > 0): ?><span class="admin-media-card__status--used">In use</span><?php endif; ?><?php if ($itemEvidence['variantCount'] > 0): ?><span><?= $esc($itemEvidence['variantCount']) ?> <?= $itemEvidence['variantCount'] === 1 ? 'variant' : 'variants' ?></span><?php endif; ?></div>
                        </div>
                    </article>
                <?php endforeach; ?>
            </div>
    ...
    <template data-media-preview-actions-template>
        ...
        <?php endif; ?>
        <?php if (!empty($canDelete)): ?><div class="admin-media-preview__destructive"><form method="post" data-preview-delete-form><input type="hidden" name="_token" value="<?= $esc($csrfToken) ?>"><button class="admin-button admin-button--danger" type="submit">Delete media</button><p class="admin-form__hint" data-preview-delete-help>Deletion is blocked automatically while this media is in use.</p></form></div><?php endif; ?>
    </template>
