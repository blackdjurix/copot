...
<?php if ($noticeText): ?><div class="admin-notice" role="status"><?= $esc($noticeText) ?></div><?php endif; ?>
<?php if ($error): ?><div class="admin-error" role="alert"><?= $esc($error) ?></div><?php endif; ?>
<form class="admin-panel admin-content-filters" method="get" action="<?= $esc($adminUrl('media')) ?>" aria-label="Media filters">
...
<?php else: ?>
    <div class="admin-table-wrap"><table class="admin-table"><thead><tr><th>Preview</th><th>Media</th><th>Details</th><th>Actions</th></tr></thead><tbody>
    <?php foreach ($mediaItems as $item): $editable = $isEditable($item); ?>
        <tr>
            <td><?php if ($item->kind() === 'image'): ?><img src="<?= $esc('/media/' . $item->id()->value()) ?>" alt="<?= $esc($item->title()) ?>" width="96" height="72" loading="lazy"><?php else: ?><span aria-label="PDF or document" class="admin-media-document">Document</span><?php endif; ?></td>
            <td><strong><?= $esc($item->title()) ?></strong><br><span><?= $esc($item->originalFilename()) ?></span><br><span><?= $esc(ucfirst($item->kind())) ?> · <?= $esc($item->mimeType()) ?></span></td>
            <td><?= $item->width() !== null ? $esc($item->width() . ' × ' . $item->height()) . '<br>' : '' ?><?= $esc($formatBytes($item->byteSize())) ?><br><span><?= $editable ? 'Editable' : 'Manage-only' ?></span><br><time datetime="<?= $esc($item->updatedAt()) ?>"><?= $esc($item->updatedAt()) ?></time></td>
            <td><div class="admin-row-actions"><a class="admin-button admin-button--link" href="<?= $esc('/media/' . $item->id()->value()) ?>">View</a><a class="admin-button admin-button--link" href="<?= $esc('/media/' . $item->id()->value() . '/download') ?>">Download</a>
