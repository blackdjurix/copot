...
final class MediaUsageRepository
{
    ...
    public function forMedia(MediaId|int $mediaId, bool $forUpdate = false): array
    {
        ...
        $statement = $this->database->prepareModule('SELECT * FROM media_usages WHERE media_id = :media_id ORDER BY consumer_type ASC, consumer_id ASC, usage_key ASC' . ($forUpdate ? ' FOR UPDATE' : ''));
    ...
    {
        $mediaId = $mediaId instanceof MediaId ? $mediaId : new MediaId($mediaId);
        $statement = $this->database->prepareModule('INSERT IGNORE INTO media_usages (media_id, consumer_type, consumer_id, usage_key, created_at) VALUES (:media_id, :consumer_type, :consumer_id, :usage_key, NOW())');
    ...
    {
        $mediaId = $mediaId instanceof MediaId ? $mediaId : new MediaId($mediaId);
        $statement = $this->database->prepareModule('DELETE FROM media_usages WHERE media_id = :media_id AND consumer_type = :consumer_type AND consumer_id = :consumer_id AND usage_key = :usage_key');
        $statement->execute(['media_id' => $mediaId->value(), 'consumer_type' => trim($consumerType), 'consumer_id' => $consumerId, 'usage_key' => trim($usageKey)]);
    }
