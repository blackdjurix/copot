...
{
    private function __construct(private MediaId $sourceId, private ?array $crop, private ?array $aspectRatio, private int $rotation, private ?int $resizeWidth, private ?int $resizeHeight, private string $fit, private ?string $outputFormat, private ?int $quality, private array $responsiveWidths) {}
    ...
    public static function fromArray(MediaId|int $sourceId, array $data): self
    {
        ...
        return new self($sourceId, $crop, $aspect, $rotation, $resizeWidth, $resizeHeight, $fit, $format, $quality, $responsive);
    }
    ...
    public function canonical(): string { return json_encode(['crop'=>$this->crop,'aspect'=>$this->aspectRatio,'rotation'=>$this->rotation,'resize'=>[$this->resizeWidth,$this->resizeHeight],'fit'=>$this->fit,'format'=>$this->outputFormat,'quality'=>$this->quality,'responsive'=>$this->responsiveWidths], JSON_THROW_ON_ERROR); }
    public function semanticKey(): string { return 'r1-' . substr(hash('sha256', $this->canonical()), 0, 32); }
    public function forWidth(int $width): self { return new self($this->sourceId, $this->crop, $this->aspectRatio, $this->rotation, $width, null, $this->fit, $this->outputFormat, $this->quality, []); }
