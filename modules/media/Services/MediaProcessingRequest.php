...
{
    private function __construct(private MediaId $sourceId, private ?array $crop, private ?array $aspectRatio, private ?array $cropAspectRatio, private bool $allowUpscale, private int $rotation, private ?int $resizeWidth, private ?int $resizeHeight, private string $fit, private ?string $outputFormat, private ?int $quality, private array $responsiveWidths, private ?string $profile = null) {}
    ...
    public static function fromArray(MediaId|int $sourceId, array $data): self
    {
        ...
        $crop = self::rect($data['crop'] ?? null, 'crop'); $aspect = self::ratio($data['aspect_ratio'] ?? null); $cropAspect = self::ratio($data['crop_aspect_ratio'] ?? null);
        if ($crop !== null && $aspect !== null) throw new MediaProcessingValidationException('Crop modes are mutually exclusive.');
        $allowUpscale = $data['allow_upscale'] ?? false; if (!is_bool($allowUpscale)) throw new MediaProcessingValidationException('Upscaling preference is invalid.');
        ...
        $responsive = array_values(array_unique(array_map(static fn ($value): int => self::positiveInt($value, 'responsive width'), $responsive))); foreach ($responsive as $width) if ($width > 4096) throw new MediaProcessingValidationException('Responsive width exceeds the processing limit.');
        ...
        $profile = $data['profile'] ?? null; if ($profile !== null && (!is_string($profile) || !preg_match('/^[a-z0-9][a-z0-9._-]{0,63}$/', $profile))) throw new MediaProcessingValidationException('Processing profile is invalid.');
        return new self($sourceId, $crop, $aspect, $cropAspect, $allowUpscale, $rotation, $resizeWidth, $resizeHeight, $fit, $format, $quality, $responsive, $profile);
    }
    ...
    public function cropAspectRatio(): ?array { return $this->cropAspectRatio; }
    public function allowUpscale(): bool { return $this->allowUpscale; }
    ...
    public function canonical(): string { return json_encode(['profile'=>$this->profile,'crop'=>$this->crop,'crop_aspect'=>$this->cropAspectRatio,'aspect'=>$this->aspectRatio,'allow_upscale'=>$this->allowUpscale,'rotation'=>$this->rotation,'resize'=>[$this->resizeWidth,$this->resizeHeight],'fit'=>$this->fit,'format'=>$this->outputFormat,'quality'=>$this->quality,'responsive'=>$this->responsiveWidths], JSON_THROW_ON_ERROR); }
    ...
    public function forWidth(int $width): self { return new self($this->sourceId, $this->crop, $this->aspectRatio, $this->cropAspectRatio, $this->allowUpscale, $this->rotation, $width, null, $this->fit, $this->outputFormat, $this->quality, [], $this->profile); }
