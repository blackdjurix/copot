...
final class FormRepository
{
    ...
    {
        $statement = $this->database->prepareModule('SELECT * FROM forms WHERE id = :id LIMIT 1' . ($forUpdate ? ' FOR UPDATE' : ''));
    ...
    {
        $statement = $this->database->prepareModule('INSERT INTO forms (name,status,created_at,updated_at) VALUES (:name,:status,NOW(),NOW())');
    ...
    public function update(FormId|int $id, string $name, string $status, ?string $expectedUpdatedAt = null): void
    {
        ...
        $statement = $this->database->prepareModule($sql); $statement->execute($parameters);
    ...
    {
        $statement = $this->database->prepareModule('DELETE FROM forms WHERE id=:id'); $statement->execute(['id' => $this->id($id)->value()]);
    ...
    {
        $statement = $this->database->prepareModule('SELECT 1 FROM form_submissions WHERE form_id=:id LIMIT 1' . ($forUpdate ? ' FOR UPDATE' : ''));
        $statement->execute(['id' => $this->id($id)->value()]); return $statement->fetchColumn() !== false;
    }
    public function allByStatus(string $status): array
    {
        $statement = $this->database->prepareModule('SELECT * FROM forms WHERE status=:status ORDER BY updated_at DESC,id DESC'); $statement->execute(['status' => $status]);
    ...
    public function workspace(array $filters = [], int $limit = 25, int $offset = 0): array
    {
        ...
        $count=$this->database->prepareModule('SELECT COUNT(*) FROM forms'.$clause);$count->execute($parameters);
        $statement=$this->database->prepareModule('SELECT * FROM forms'.$clause.' ORDER BY updated_at DESC,id DESC LIMIT :limit OFFSET :offset');
...
final class FormFieldRepository
{
    ...
    public function forForm(FormId|int $formId, bool $forUpdate = false): array
    {
        ...
        $statement=$this->database->prepareModule($sql); $statement->execute(['form_id'=>$id->value()]); $fields=[];
    ...
    public function replace(FormId|int $formId, array $definitions): void
    {
        ...
        $existing=$this->database->prepareModule('SELECT id,field_key FROM form_fields WHERE form_id=:form_id FOR UPDATE'); $existing->execute(['form_id'=>$formId->value()]);
        $byKey=[]; foreach($existing->fetchAll() as $row) $byKey[(string)$row['field_key']]=(int)$row['id']; $wanted=[];
        if ($byKey !== []) $this->database->prepareModule('UPDATE form_fields SET sort_order = sort_order + 1000001 WHERE form_id = :form_id')->execute(['form_id' => $formId->value()]);
        foreach ($definitions as $definition) {
            ...
            if (isset($byKey[$definition['field_key']])) {
                ...
                $update = $this->database->prepareModule('UPDATE form_fields SET label=:label,field_type=:field_type,sort_order=:sort_order,is_required=:is_required,min_length=:min_length,max_length=:max_length,updated_at=GREATEST(NOW(),DATE_ADD(updated_at,INTERVAL 1 SECOND)) WHERE id=:id');
                $update->execute([...array_diff_key($fieldParameters, ['field_key' => true]), 'id' => $fieldId]);
                $this->database->prepareModule('DELETE FROM form_field_options WHERE form_field_id=:id')->execute(['id'=>$fieldId]);
            }
            else { $insert=$this->database->prepareModule('INSERT INTO form_fields (form_id,field_key,label,field_type,sort_order,is_required,min_length,max_length,created_at,updated_at) VALUES (:form_id,:field_key,:label,:field_type,:sort_order,:is_required,:min_length,:max_length,NOW(),NOW())'); $insert->execute(['form_id'=>$formId->value(),...$fieldParameters]); $fieldId=(int)$connection->lastInsertId(); }
            $option=$this->database->prepareModule('INSERT INTO form_field_options (form_field_id,option_value,option_label,sort_order,created_at,updated_at) VALUES (:form_field_id,:option_value,:option_label,:sort_order,NOW(),NOW())');
            foreach ($definition['options'] as $value) $option->execute(['form_field_id'=>$fieldId,...$value]);
        }
        foreach($byKey as $key=>$fieldId) if (!isset($wanted[$key])) $this->database->prepareModule('DELETE FROM form_fields WHERE id=:id')->execute(['id'=>$fieldId]);
    }
    /** @return FormFieldOption[] */
    private function options(FormFieldId $id): array
    {
        $statement=$this->database->prepareModule('SELECT * FROM form_field_options WHERE form_field_id=:id ORDER BY sort_order ASC,option_value ASC'); $statement->execute(['id'=>$id->value()]);
...
final class FormSubmissionRepository
{
    ...
    {
        $formId=$this->formId($formId); $connection=$this->database->connection();
        $statement=$this->database->prepareModule("INSERT INTO form_submissions (form_id,status,created_at,updated_at) VALUES (:form_id,'new',NOW(),NOW())"); $statement->execute(['form_id'=>$formId->value()]); $id=new FormSubmissionId((int)$connection->lastInsertId());
        $insert=$this->database->prepareModule('INSERT INTO form_submission_values (submission_id,form_field_id,field_key,field_label,field_type,value_text,value_label,created_at) VALUES (:submission_id,:form_field_id,:field_key,:field_label,:field_type,:value_text,:value_label,NOW())');
    ...
    {
        $id=$this->id($id); $statement=$this->database->prepareModule('SELECT * FROM form_submissions WHERE id=:id LIMIT 1'.($forUpdate?' FOR UPDATE':'')); $statement->execute(['id'=>$id->value()]); $row=$statement->fetch(); return is_array($row)?$this->hydrate($row):null;
    }
    ...
    public function workspace(array $filters = [], int $limit = 25, int $offset = 0): array
    {
        ...
        $clause=$where===[]?'':' WHERE '.implode(' AND ',$where);$connection=$this->database->connection();
        $count=$this->database->prepareModule('SELECT COUNT(*) FROM form_submissions s INNER JOIN forms f ON f.id=s.form_id'.$clause);$count->execute($parameters);
        $statement=$this->database->prepareModule('SELECT s.id,s.form_id,f.name AS form_name,s.status,s.created_at,s.updated_at FROM form_submissions s INNER JOIN forms f ON f.id=s.form_id'.$clause.' ORDER BY s.updated_at DESC,s.id DESC LIMIT :limit OFFSET :offset');
        foreach($parameters as $key=>$value)$statement->bindValue(':'.$key,$value);$statement->bindValue(':limit',$limit,PDO::PARAM_INT);$statement->bindValue(':offset',$offset,PDO::PARAM_INT);$statement->execute();
    ...
    {
        $statement=$this->database->prepareModule('UPDATE form_submissions SET status=:status,updated_at=GREATEST(NOW(),DATE_ADD(updated_at,INTERVAL 1 SECOND)) WHERE id=:id'); $statement->execute(['id'=>$this->id($id)->value(),'status'=>$status]); if($statement->rowCount()!==1) throw new FormSubmissionNotFoundException('Submission is unavailable.');
    }
    public function delete(FormSubmissionId|int $id): void
    {
        $statement=$this->database->prepareModule('DELETE FROM form_submissions WHERE id=:id'); $statement->execute(['id'=>$this->id($id)->value()]); if($statement->rowCount()!==1) throw new FormSubmissionNotFoundException('Submission is unavailable.');
    }
    private function values(FormSubmissionId $id): array
    {
        $statement=$this->database->prepareModule('SELECT * FROM form_submission_values WHERE submission_id=:id ORDER BY id ASC'); $statement->execute(['id'=>$id->value()]);
