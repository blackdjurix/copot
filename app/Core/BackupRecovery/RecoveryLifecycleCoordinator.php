...
final class RecoveryLifecycleCoordinator
{
    ...
    { $record = $this->store->read($identity); if (!$record->confirmationMatches($identity, $record->manifestIdentity(), $targetIdentity)) throw new RecoveryLifecycleException('Cleanup confirmation does not match the recovery set.'); return $this->store->transition($identity, RecoveryLifecycleState::CLEANUP_PENDING); }
