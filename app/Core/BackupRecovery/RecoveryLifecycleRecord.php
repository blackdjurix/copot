...
final class RecoveryLifecycleRecord
{
    public function __construct(
        ...
        private bool $postReconciliationVerified = false
    ) {
    ...
    {
        $required = ['recovery_identity','manifest_identity','operation_identity','state','mutation_started','capture_complete','confirmation_recovery_identity','confirmation_manifest_identity','confirmation_target_identity','restore_attempt_identity','restore_stage','failure_class','failure_reason','created_at','updated_at','post_reconciliation_verified'];
        if (array_keys($data) !== $required || !is_bool($data['mutation_started']) || !is_bool($data['capture_complete']) || !is_bool($data['post_reconciliation_verified'])) {
    ...
    public function postReconciliationVerified(): bool { return $this->postReconciliationVerified; }
    ...
    public function transition(string $state, string $reason = ''): self
    {
        ...
        return $this->copy($state, $this->mutationStarted, $this->captureComplete, $this->confirmationRecoveryIdentity, $this->confirmationManifestIdentity, $this->confirmationTargetIdentity, $this->restoreAttemptIdentity, $this->restoreStage, $this->failureClass, $reason, $this->postReconciliationVerified);
    }

    public function withCaptureComplete(): self { return $this->copy($this->state, $this->mutationStarted, true, $this->confirmationRecoveryIdentity, $this->confirmationManifestIdentity, $this->confirmationTargetIdentity, $this->restoreAttemptIdentity, $this->restoreStage, $this->failureClass, $this->failureReason, $this->postReconciliationVerified); }
    public function withConfirmation(RecoveryIdentity $recovery, string $manifest, string $target): self
    { self::hash($manifest, 'Confirmation manifest identity'); return $this->copy($this->state, $this->mutationStarted, $this->captureComplete, $recovery->value(), $manifest, $target, $this->restoreAttemptIdentity, $this->restoreStage, $this->failureClass, $this->failureReason, $this->postReconciliationVerified); }
    public function withMutationStarted(): self { return $this->copy($this->state, true, $this->captureComplete, $this->confirmationRecoveryIdentity, $this->confirmationManifestIdentity, $this->confirmationTargetIdentity, $this->restoreAttemptIdentity, $this->restoreStage, $this->failureClass, $this->failureReason, $this->postReconciliationVerified); }
    public function withRestoreStage(string $attempt, string $stage): self { return $this->copy($this->state, $this->mutationStarted, $this->captureComplete, $this->confirmationRecoveryIdentity, $this->confirmationManifestIdentity, $this->confirmationTargetIdentity, $attempt, $stage, $this->failureClass, $this->failureReason, $this->postReconciliationVerified); }
    public function withPostReconciliationVerified(): self { return $this->copy($this->state, $this->mutationStarted, $this->captureComplete, $this->confirmationRecoveryIdentity, $this->confirmationManifestIdentity, $this->confirmationTargetIdentity, $this->restoreAttemptIdentity, $this->restoreStage, $this->failureClass, $this->failureReason, true); }

    private function copy(string $state, bool $mutation, bool $capture, ?string $confirmRecovery, ?string $confirmManifest, ?string $confirmTarget, ?string $attempt, ?string $stage, string $failureClass, string $reason, bool $postVerified): self
    { return new self($this->recoveryIdentity, $this->manifestIdentity, $this->operationIdentity, $state, $mutation, $capture, $confirmRecovery, $confirmManifest, $confirmTarget, $attempt, $stage, $failureClass, $reason, $this->createdAt, gmdate(DATE_ATOM), $postVerified); }
