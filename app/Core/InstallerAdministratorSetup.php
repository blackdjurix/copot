...
class InstallerAdministratorSetup
{
    ...
    public function install(array $input, bool $requirementsPassed): array
    {
        ...
        try {
            return $this->installPrepared($input, $requirementsPassed);
    ...
    /** Execute the administrator mutation while the caller owns the installation lock. */
    public function installPrepared(array $input, bool $requirementsPassed): array
    {
        if (!$requirementsPassed) {
            throw new InstallationException('Installer requirements are not satisfied.');
        }
        ...
        try {
            ...
            if ($this->users->findByEmail($values['email']) instanceof User) {
                throw new InstallerValidationException(['admin_email' => 'Administrator email is already in use.'], $this->submittedValues($values));
            }
            if ($this->userCount() !== 0) {
            ...
            try {
                $statement = $connection->prepare('INSERT INTO ' . $this->database->table('users') . ' (name, email, password_hash, status, created_at, updated_at) VALUES (:name, :email, :password_hash, :status, NOW(), NOW())');
                $statement->execute(['name' => $values['name'], 'email' => $values['email'], 'password_hash' => $passwordHash, 'status' => 'active']);
                $userId = (int) $connection->lastInsertId();
                $statement = $connection->prepare('INSERT INTO ' . $this->database->table('user_roles') . ' (user_id, role_id) VALUES (:user_id, :role_id)');
                ...
                $connection->commit();
                return ['user_id' => $userId, 'email' => $values['email']];
            } catch (PDOException) {
