    ...
    <div class="admin-panel__body">
        ...
        <form method="post" action="<?= $escape($path) ?>" class="admin-form admin-settings-form" id="site-settings-identity-form" data-settings-dirty-form>
            ...
            <fieldset class="admin-fieldset"><legend class="admin-fieldset__legend">General</legend>
                <div class="admin-settings-field-grid"><?php foreach ([['site_name','Site Name','name'],['site_tagline','Site Tagline','tagline']] as [$name,$label,$key]): ?><div class="admin-field"><label class="admin-field__label" for="<?= $name ?>"><?= $escape($label) ?></label><input id="<?= $name ?>" name="<?= $name ?>" type="text" value="<?= $escape($values[$key] ?? '') ?>" <?= $key === 'name' ? 'required' : '' ?>><p class="admin-field__error"><?= $escape($errors['site.' . $key] ?? '') ?></p></div><?php endforeach; ?></div>
            </fieldset>
            <fieldset class="admin-fieldset"><legend class="admin-fieldset__legend">Homepage</legend><div class="admin-settings-field-grid"><div class="admin-field"><label class="admin-field__label" for="hero_media">Hero Image</label><select id="hero_media" name="hero_media"><option value="">No Hero Image</option><?php foreach ($media as $item): ?><option value="<?= $escape($item->id()->value()) ?>" <?= (int) ($values['hero_media'] ?? 0) === $item->id()->value() ? 'selected' : '' ?>><?= $escape($item->title()) ?> — <?= $escape($item->originalFilename()) ?></option><?php endforeach; ?></select><p class="admin-field__help">Selected through Core Media; referenced media cannot be deleted.</p><p class="admin-field__error"><?= $escape($errors['hero_media'] ?? '') ?></p></div></div></fieldset>
            <fieldset class="admin-fieldset"><legend class="admin-fieldset__legend">Localization</legend><div class="admin-settings-field-grid"><?php foreach ([['locale','Locale'],['timezone','Timezone'],['date_format','Date Format'],['time_format','Time Format']] as [$key,$label]): ?><div class="admin-field"><label class="admin-field__label" for="<?= $key ?>"><?= $escape($label) ?></label><input id="<?= $key ?>" name="<?= $key ?>" type="text" value="<?= $escape($values[$key] ?? '') ?>" required></div><?php endforeach; ?></div></fieldset>
            <fieldset class="admin-fieldset"><legend class="admin-fieldset__legend">Appearance</legend><div class="admin-settings-field-grid"><div class="admin-field"><label class="admin-field__label" for="main_color">Webcore Color Scheme — Main Color</label><input id="main_color" name="main_color" type="color" value="<?= $escape($values['main_color'] ?? '#1769e0') ?>" required><p class="admin-field__help">System-derived shades and Webcore-owned black/white neutral bases are generated automatically.</p></div></div></fieldset>
        </form>
        ...
        <div class="admin-actions admin-form__actions"><button class="admin-button admin-button--primary" type="submit" form="site-settings-identity-form">Save Site Settings</button></div>
        </section>
