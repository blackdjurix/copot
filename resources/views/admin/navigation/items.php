...
$renderTree = function (array $nodes) use (&$renderTree, $esc, $selectedId, $navigationPath, $itemPath, $csrfToken): void {
    foreach ($nodes as $node) {
        ...
        echo '<span class="admin-navigation-tree__actions"><a class="admin-button admin-button--link" href="' . $esc($navigationPath . '?item=' . $item->id()) . '">Edit</a><form class="admin-inline-form" method="post" action="' . $esc($itemPath('/' . $item->id() . '/delete')) . '"><input type="hidden" name="_token" value="' . $esc($csrfToken) . '"><button class="admin-button admin-button--link admin-action-danger" type="submit">Delete</button></form></span>';
        echo '</div>';
...
<div class="admin-navigation-workspace" data-navigation-workspace>
    ...
    <section class="admin-panel admin-navigation-workspace__detail" aria-labelledby="navigation-detail-title">
        ...
        <div class="admin-panel__body">
            ...
            <form class="admin-form" method="post" action="<?= $esc($formAction) ?>" <?= !$selected && !$creating ? 'aria-disabled="true"' : '' ?>><input type="hidden" name="_token" value="<?= $esc($csrfToken) ?>">
                ...
                <?php endif; ?>
                <div class="admin-form__actions"><?php if ($selected || $creating): ?><a class="admin-button admin-button--secondary" href="<?= $esc($navigationPath) ?>">Back to tree</a><?php endif; ?><button class="admin-button admin-button--primary" type="submit" <?= !$selected && !$creating ? 'disabled' : '' ?>>Save</button></div>
            </form>
        </div>
    </section>
</div>
