    ...
    <section class="admin-panel admin-navigation-workspace__detail" aria-labelledby="navigation-detail-title">
        ...
        <div class="admin-panel__body">
            ...
            <form class="admin-form" method="post" action="<?= $esc($formAction) ?>" <?= !$selected && !$creating ? 'aria-disabled="true"' : '' ?>><input type="hidden" name="_token" value="<?= $esc($csrfToken) ?>">
                ...
                <div class="admin-form__actions"><a class="admin-button admin-button--secondary" href="<?= $esc($navigationPath) ?>">Back to tree</a><button class="admin-button admin-button--primary" type="submit" <?= !$selected && !$creating ? 'disabled' : '' ?>>Save</button></div>
            </form>
        </div>
    </section>
</div>
