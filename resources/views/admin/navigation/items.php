    ...
    <section class="admin-panel admin-navigation-workspace__detail" aria-labelledby="navigation-detail-title">
        ...
        <div class="admin-panel__body">
            ...
            <?php if ($selected && !$editing && !$creating): ?><dl class="admin-navigation-preview"><div><dt>Label</dt><dd><?= $esc($formItem['label']) ?></dd></div><div><dt>Target</dt><dd><?= $esc(ucwords(str_replace('_', ' ', $formItem['target_choice']))) ?></dd></div><div><dt>Visibility</dt><dd><?= !empty($formItem['is_visible']) ? 'Visible' : 'Hidden' ?></dd></div></dl><div class="admin-form__actions"><a class="admin-button admin-button--primary" href="<?= $esc($navigationPath . '?item=' . $selected->id() . '&edit=1') ?>">Edit</a><form data-navigation-delete method="post" action="<?= $esc($itemPath('/' . $selected->id() . '/delete')) ?>"><input type="hidden" name="_token" value="<?= $esc($csrfToken) ?>"><button class="admin-button admin-button--danger" type="submit">Delete</button></form></div><?php else: ?><form class="admin-form" data-navigation-edit-form method="post" action="<?= $esc($formAction) ?>" <?= !$selected && !$creating ? 'aria-disabled="true"' : '' ?>><input type="hidden" name="_token" value="<?= $esc($csrfToken) ?>">
                ...
                <?php endif; ?>
                <div class="admin-form__actions"><a class="admin-button admin-button--secondary" data-navigation-cancel href="<?= $esc($navigationPath . ($selected ? '?item=' . $selected->id() : '')) ?>">Cancel</a><button class="admin-button admin-button--primary" type="submit" <?= !$selected && !$creating ? 'disabled' : '' ?>>Save</button></div>
            </form><?php endif; ?>
        </div>
    </section>
</div>
