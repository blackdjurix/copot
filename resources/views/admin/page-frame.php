...
<section class="admin-page-frame admin-page-frame--surface-<?= htmlspecialchars($surface, ENT_QUOTES, 'UTF-8') ?> admin-page-frame--spacing-<?= htmlspecialchars($spacing, ENT_QUOTES, 'UTF-8') ?>" aria-labelledby="<?= htmlspecialchars($titleId, ENT_QUOTES, 'UTF-8') ?>"<?= $descriptionId !== null ? ' aria-describedby="' . htmlspecialchars($descriptionId, ENT_QUOTES, 'UTF-8') . '"' : '' ?>>
    <header class="admin-page-frame__header">
        <div class="admin-page-frame__heading">
            <h2 class="admin-page-frame__title" id="<?= htmlspecialchars($titleId, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($title, ENT_QUOTES, 'UTF-8') ?></h2>
            <?php if ($description !== null): ?><p class="admin-page-frame__description" id="<?= htmlspecialchars($descriptionId, ENT_QUOTES, 'UTF-8') ?>"><?= htmlspecialchars($description, ENT_QUOTES, 'UTF-8') ?></p><?php endif; ?>
        </div>
        <?php if ($bar !== null): ?><div class="admin-page-frame__bar"><?= $bar ?></div><?php endif; ?>
    </header>
