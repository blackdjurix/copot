...
<div class="system-manager-workspace" data-system-manager>
    ...
    <?php elseif ($activeSection === 'branding'): ?>
        <section class="admin-panel system-manager-panel" aria-labelledby="system-manager-localization-title">
            ...
            <div class="admin-panel__body"><form method="post" action="<?= $escape($localizationPath ?? '') ?>" class="system-manager-form system-manager-detail-form"><input type="hidden" name="_token" value="<?= $escape($csrfToken ?? '') ?>"><label class="system-manager-detail-row"><span class="admin-field__label">Locale</span><select name="locale"><option value="en_US"<?= ($localization['locale'] ?? '') === 'en_US' ? ' selected' : '' ?>>en_US</option><option value="id_ID"<?= ($localization['locale'] ?? '') === 'id_ID' ? ' selected' : '' ?>>id_ID</option></select></label><label class="system-manager-detail-row"><span class="admin-field__label">Timezone</span><select name="timezone"><?php foreach (timezone_identifiers_list() as $timezone): ?><option value="<?= $escape($timezone) ?>"<?= ($localization['timezone'] ?? '') === $timezone ? ' selected' : '' ?>><?= $escape($timezone) ?></option><?php endforeach; ?></select></label><label class="system-manager-detail-row"><span class="admin-field__label">Date Format</span><select name="date_format"><?php foreach (['Y-m-d', 'd/m/Y', 'm/d/Y', 'd M Y'] as $format): ?><option value="<?= $escape($format) ?>"<?= ($localization['date_format'] ?? '') === $format ? ' selected' : '' ?>><?= $escape($format) ?></option><?php endforeach; ?></select></label><label class="system-manager-detail-row"><span class="admin-field__label">Time Format</span><select name="time_format"><?php foreach (['H:i', 'h:i A'] as $format): ?><option value="<?= $escape($format) ?>"<?= ($localization['time_format'] ?? '') === $format ? ' selected' : '' ?>><?= $escape($format) ?></option><?php endforeach; ?></select></label><button class="admin-button admin-button--primary" type="submit">Save Localization</button></form></div>
        </section>
        <section class="admin-panel system-manager-panel" aria-labelledby="system-manager-branding-title">
            ...
            <div class="admin-panel__body"><form method="post" action="<?= $escape($brandingPath ?? '') ?>" class="system-manager-form system-manager-detail-form system-manager-branding-form"><input type="hidden" name="_token" value="<?= $escape($csrfToken ?? '') ?>"><?php foreach (['main' => 'Main', 'accent' => 'Accent', 'neutral-dark' => 'Neutral Dark', 'neutral-light' => 'Neutral Light'] as $key => $label): ?><div class="system-manager-detail-row" data-color-control><span class="admin-field__label"><?= $escape($label) ?></span><span class="system-manager-color-control"><input class="system-manager-color-native" type="color" value="<?= $escape($palette[$key] ?? '') ?>" data-color-native aria-label="<?= $escape($label) ?> swatch"><select data-color-format aria-label="<?= $escape($label) ?> color format"><option value="hex">HEX</option><option value="rgb">RGB</option><option value="hsl">HSL</option></select><input type="text" value="<?= $escape($palette[$key] ?? '') ?>" data-color-input inputmode="text" aria-label="<?= $escape($label) ?> color value"><input type="hidden" name="<?= $escape($key) ?>" value="<?= $escape($palette[$key] ?? '') ?>" data-color-canonical></span></div><?php endforeach; ?><label class="system-manager-detail-row"><span class="admin-field__label">Admin identity display</span><select name="identity_mode"><option value="text"<?= ($branding['identity_mode'] ?? '') === 'text' ? ' selected' : '' ?>>Text — Site Name</option><option value="logo"<?= ($branding['identity_mode'] ?? '') === 'logo' ? ' selected' : '' ?>>Logo</option></select></label><label class="system-manager-detail-row"><span class="admin-field__label">Admin text color</span><select name="identity_color"><?php foreach (['main' => 'Main', 'accent' => 'Accent', 'neutral-dark' => 'Neutral Dark', 'neutral-light' => 'Neutral Light'] as $key => $label): ?><option value="<?= $escape($key) ?>"<?= ($branding['identity_color'] ?? '') === $key ? ' selected' : '' ?>><?= $escape($label) ?></option><?php endforeach; ?></select></label><p class="admin-field__help">Required foreground relationships use contrast-aware Neutral Dark / Neutral Light resolution.</p><button class="admin-button admin-button--primary" type="submit">Save Branding</button></form></div>
        </section>
