(function () {
    ...
    workspace.querySelectorAll('[data-navigation-row][draggable="true"]').forEach(function (row) {
        ...
        row.addEventListener('dragover', function (event) { if (!dragged || dragged === row.dataset.navigationId) return; event.preventDefault(); clearDropState(); var plan = dropPlan(row, event); row.classList.add('is-drop-target', 'is-drop-' + (plan.parent === row.dataset.navigationId ? 'inside' : ((event.clientY - row.getBoundingClientRect().top) < row.getBoundingClientRect().height * .5 ? 'before' : 'after'))); if (plan.parent === row.dataset.navigationId && row.closest('[data-navigation-node]').classList.contains('is-collapsed')) { expandTimer = window.setTimeout(function () { var toggle = row.closest('[data-navigation-node]').querySelector('[data-navigation-toggle]'); if (toggle) toggle.click(); }, 600); } });
